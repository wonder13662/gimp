module.exports = {
  en2023: new Map(
    [
      [
        '00-home.md', 
        '0. Home',
        '최상위'
      ],
      [
        '01-00-introduction.md', 
        '1. Introduction',
        '1. 소개',
      ],
      [
        '01-01-welcome-to-gimp.md', 
        '1.1. Welcome to GIMP',
        '1.1. GIMP(김프)에 오신 것을 환영합니다'
      ],
      [
        '01-02-whats-new-in-gimp.md', 
        '1.2. What\'s new in GIMP',
        '1.2. GIMP(김프)의 새로운 기능 알아보기'
      ],
      [
        '02-00-fire-up-gimp.md', 
        '2. Fire up GIMP',
        '2. GIMP(김프) 시작하기'
      ],
      [
        '02-01-00-running-gimp.md', 
        '2.1. Running GIMP',
        '2.1. GIMP(김프) 실행하기'
      ],
      [
        '02-02-configuration-folders.md', 
        '2.2. Configuration folders',
        '2.2. 설정 폴더들'
      ],
      [
        '03-00-first-step-with-gimp.md', 
        '3. First step with GIMP',
        '3. GIMP(김프)의 첫번째 단계'
      ],
      [
        '03-01-basic-concepts.md', 
        '3.1. Basic concepts',
        '3.1. GIMP(김프)의 기본 개념들',
      ],
      [
        '03-02-00-main-window.md', 
        '3.2. Main window',
        '3.2. GIMP(김프)의 주요 화면',
      ],
      [
        '03-02-03-the-toolbox.md', 
        '3.2.1. The toolbox',
        '3.2.3. 도구상자(The toolbox)',
      ],
      [
        '03-02-04-00-image_window.md',
        '3.2.2. Image window',
        '3.2.4. 이미지 창(Image window)',
      ],
      [
        '03-02-05-00-dialogs-and-docking.md', 
        '3.2.3. Dialogs and docking',
        '3.2.5. 대화상자(Dialog)와 도킹(docking)',
      ],
      [
        '03-03-00-undoing.md', 
        '3.3. Undoing',
        '3.3. 실행 취소하기',
      ],
      [
        '03-04-00-common-tasks.md', 
        '3.4. Common tasks',
        '3.4. 자주하는 편집 작업',
      ],
      [
        '03-04-01-intention.md', 
        '3.4.1. Intention',
        '3.4.1. 목적',
      ],
      [
        '03-04-02-change-the-size-of-an-image-for-the-screen.md', 
        '3.4.2. Change the size of an image for the screen',
        '3.4.2. 이미지의 크기 바꾸기 (화면용)',
      ],
      [
        '03-04-03-change-the-size-of-an-image-for-print.md', 
        '3.4.3. Change the size of an Image for print',
        '3.4.3. 이미지의 크기 바꾸기 (인쇄용)',
      ],
      [
        '03-04-04-compressing-images.md', 
        '3.4.4. Compressing images',
        '3.4.4. 이미지 압축하기',
      ],
      [
        '03-04-05-crop-an-image.md', 
        '3.4.5. Crop an image',
        '3.4.5. 이미지 잘라내기',
      ],
      [
        '03-04-06-find-info-about-your-image.md', 
        '3.4.6. Find info about your image',
        '3.4.6. 이미지 정보 확인하기',
      ],
      [
        '03-04-07-change-the-mode.md', 
        '3.4.7. Change the mode',
        '3.4.7. 모드 바꾸기',
      ],
      [
        '03-04-08-flip-an-image.md', 
        '3.4.8. Flip an image',
        '3.4.8. 이미지 뒤집기',
      ],
      [
        '03-04-09-rotate-an-image.md', 
        '3.4.9. Rotate an image',
        '3.4.9. 이미지 회전하기'
      ],
      [
        '03-04-10-separating-an-object-from-its-background.md', 
        '3.4.10. Separating an object from its background',
        '3.4.10. 원하는 개체만 선택하여 분리하기',
      ],
      [
        '03-05-00-how-to-draw-straight-lines.md', 
        '3.5. How to draw straight lines',
        '3.5. 직선 그리기',
      ],
      [
        '03-05-01-intention.md', 
        '3.5.1. Intention',
        '3.5.1. 목적',
      ],
      [
        '03-05-02-examples.md', 
        '3.5.2. Examples',
        '3.5.2. 직선그리기 예제',
      ],
      [
        '04-00-what-to-do-if-you-are-stuck.md', 
        '4. What to do if you are stuck',
        '4. 문제 해결하기',
      ],
      [
        '04-01-introduction.md', 
        '4.1. Introduction',
        '4.1. 소개',
      ],
      [
        '04-02-00-common-causes-of-gimp-non-responsiveness.md', 
        '4.2. Common causes of GIMP non responsiveness',
        '4.2. GIMP(김프)가 멈추었을 때의 원인',
      ],
      [
        '04-02-01-there-is-a-floating-selection.md', 
        '4.2.1. There is a floating selection',
        '4.2.1. 떠있는 선택(floating selection)',
      ],
      [
        '04-02-02-the-selection-is-hidden.md', 
        '4.2.2. The selection is hidden',
        '4.2.2. 숨겨진 선택',
      ],
      [
        '04-02-03-you-are-acting-outside-the-selection.md', 
        '4.2.3. You are acting outside the selection',
        '4.2.3. 선택영역 바깥쪽에서 작업을 하려는 경우',
      ],
      [
        '04-02-04-the-active-drawable-is-not-visible.md', 
        '4.2.4. The active drawable is not visible',
        '4.2.4. 활성화된 drawable이 보이지 않는 경우',
      ],
      [
        '04-02-05-the-active-drawable-is-tranparent.md', 
        '4.2.5. The active drawable is tranparent',
        '4.2.5. 활성화된 drawable이 투명한 경우'
      ],
      [
        '04-02-06-you-are-trying-to-act-outside-the-layer.md', 
        '4.2.6. You are trying to act outside the layer',
        '4.2.6. 레이어 바깥쪽에서 작업을 하려는 경우'
      ],
      [
        '04-02-07-you-are-trying-to-act-on-a-layer-group.md', 
        '4.2.7. You are trying to act on a layer group',
        '4.2.7. 레이어 그룹에서 작업을 하려는 경우'
      ],
      [
        '04-02-08-the-image-is-in-indexed-color-mode.md', 
        '4.2.8. The image is in indexed color mode',
        '4.2.8. 인덱스 모드에서의 이미지'
      ],
      [
        '04-02-09-no-visible-effect-when-trying-to-use-a-brush-eraser-or-other-tool.md', 
        '4.2.9. No visible effect when trying to use a brush eraser or other tool',
        '4.2.9. 붓, 지우개 또는 다른 도구로 작업한 효과가 보이지 않는 경우'
      ],
      [
        '04-02-10-no-visible-effect-when-trying-to-use-the-move-tool-rotate-or-other-transform-tool.md', 
        '4.2.10. No visible effect when trying to use the move tool rotate or other transform–tool',
        '4.2.10. 이동, 회전 또는 다른 변형 도구로 작업한 효과가 보이지 않는 경우'
      ],
      [
        '04-02-11-eraser-and-brushes-no-longer-work.md', 
        '4.2.11. Eraser and brushes no longer work',
        '4.2.11. 지우개와 붓이 작동하지 않습니다'
      ],
      [
        '04-02-12-eraser-does-not-make-area-transparent.md', 
        '4.2.12. Eraser does not make area transparent',
        '4.2.12. 지우개로 지운 곳이 투명하지 않습니다'
      ],
      [
        '04-02-13-unexpected-colors-when-trying-to-use-a-brush-or-eraser.md', 
        '4.2.13. Unexpected colors when trying to use a brush or eraser',
        '4.2.13. 붓 또는 지우개를 사용하면 이상한 색이 나옵니다'
      ],
      [
        '04-02-14-the-crop-tool-leaves-an-empty-area-after-cropping.md', 
        '4.2.14. The crop tool leaves an empty area after cropping',
        '4.2.14. 잘라내기 도구로 잘라낸 뒤에 빈 공간이 생깁니다'
      ],
      [
        '04-02-15-i-ve-been-waiting-for-a-long-time-and-gimp-is-not-responding.md', 
        '4.2.15. I\'ve been waiting for a long time and GIMP is not responding',
        '4.2.15. 오랫동안 기다려도 GIMP가 반응하지 않습니다'
      ],
      [
        '04-02-16-general-guidelines-on-what-to-check-if-you-are-stuck.md', 
        '4.2.16. General guidelines on what to check if you are stuck',
        '4.2.16. 문제가 생겼을 때 확인해보면 좋은 것들'
      ],
      [
        '04-03-00-how-to-fix-missing-windows-and-dialogs.md', 
        '4.3. How to fix missing windows and dialogs',
        '4.3. 잃어버린 창과 대화상자 해결하기',
      ],
      [
        '04-03-01-all-tool-windows-are-missing.md', 
        '4.3.1. All tool windows are missing',
        '4.3.1. 모든 도구창이 사라졌습니다',
      ],
      [
        '04-03-02-tool-options-dialog-is-missing.md', 
        '4.3.2. Tool options dialog is missing',
        '4.3.2. 도구 옵션 대화상자가 사라졌습니다',
      ],
      [
        '04-03-03-some-of-the-tool-icons-are-missing.md', 
        '4.3.3. Some of the tool icons are missing',
        '4.3.3. 도구 아이콘이 사라졌습니다',
      ],
      [
        '04-03-04-the-area-showing-the-opened-images-at-the-top-is-missing.md', 
        '4.3.4. The area showing the opened images at the top is missing',
        '4.3.4. 이미지 창의 위쪽의 탭 바의 미리보기가 사라졌습니다',
      ],
      [
        '04-04-00-how-to-fix-problems-exporting-images.md', 
        '4.4. How to fix problems exporting images',
        '4.4. 이미지 내보내기 할 때의 문제 해결하기',
      ],
      [
        '04-04-01-i-am-exporting-to-a-jpeg-image-and-my-transparent-area-turned-white-or-black.md', 
        '4.4.1. I am exporting to a jpeg image and my transparent area turned white or black',
        '4.4.1. jpeg 이미지로 내보냈더니 투명 부분이 흰색 또는 검은색으로 바뀌었습니다',
      ],
      [
        '04-04-02-i-am-exporting-to-a-gif-image-and-the-colors-changed.md', 
        '4.4.2. I am exporting to a gif image and the colors changed'
      ],
      [
        '05-00-getting-images-into-gimp.md', 
        '5. Getting images into GIMP',
        '5. GIMP(김프)로 이미지 불러오기',
      ],
      [
        '05-01-image-types.md', 
        '5.1. Image types'
      ],
      [
        '05-02-creating-new-files.md', 
        '5.2. Creating new files'
      ],
      [
        '05-03-00-opening-files.md', 
        '5.3. Opening files'
      ],
      [
        '05-03-01-00-open-file.md', 
        '5.3.1. Open file'
      ],
      [
        '05-03-02-open-location.md', 
        '5.3.2. Open location'
      ],
      [
        '05-03-03-open-recent.md', 
        '5.3.3. Open recent'
      ],
      [
        '05-03-04-using-external-programs.md', 
        '5.3.4. Using external programs'
      ],
      [
        '05-03-05-file-manager.md', 
        '5.3.5. File manager'
      ],
      [
        '05-03-06-drag-and-drop.md', 
        '5.3.6. Drag and drop'
      ],
      [
        '05-03-07-copy-and-paste.md', 
        '5.3.7. Copy and paste'
      ],
      [
        '05-03-08-image-browser.md', 
        '5.3.8. Image browser'
      ],
      [
        '06-00-getting-images-out-of-gimp.md', 
        '6. Getting images out of GIMP'
      ],
      [
        '06-01-00-files.md', 
        '6.1. Files'
      ],
      [
        '06-02-00-preparing-your-images-for-the-web.md', 
        '6.2. Preparing your images for the web'
      ],
      [
        '06-02-01-images-with-an-optimal-size-quality-ratio.md', 
        '6.2.1. Images with an optimal size/quality ratio'
      ],
      [
        '06-02-02-reducing-the-file-size-even-more.md', 
        '6.2.2. Reducing the file size even more'
      ],
      [
        '06-02-03-saving-images-with-transparency.md', 
        '6.2.3. Saving images with transparency'
      ],
      [
        '07-00-painting-with-gimp.md', 
        '7. Painting with GIMP',
        '7. GIMP(김프)로 그리기'
      ],
      [
        '07-01-00-the-selection.md', 
        '7.1. The selection',
        '7.1. 선택'
      ],
      [
        '07-02-00-creating-and-using-selections.md', 
        '7.2. Creating and using selections',
        '7.2. 선택 만들기와 사용하기'
      ],
      [
        '07-02-01-00-moving-or-resizing-a-selection.md', 
        '7.2.1. Moving or resizing a selection',
        '7.2.1. 선택 영역 옮기기 또는 크기 조절하기',
      ],
      [
        '07-02-02-adding-or-subtracting-selections.md', 
        '7.2.2. Adding or subtracting selections'
      ],
      [
        '07-03-00-the-quickmask.md', 
        '7.3. The quickmask',
        '7.3. 퀵 마스크'
      ],
      [
        '07-03-01-overview.md', 
        '7.3.1. Overview',
        '7.3.1. 퀵 마스크 개요'
      ],
      [
        '07-03-02-properties.md', 
        '7.3.2. Properties'
      ],
      [
        '07-04-using-quickmask-mode.md', 
        '7.4. Using quickmask mode'
      ],
      [
        '07-05-00-paths.md', 
        '7.5. Paths',
        '7.5. 경로',
      ],
      [
        '07-05-01-path-creation.md', 
        '7.5.1. Path creation',
        '7.5.1. 경로 만들기',
      ],
      [
        '07-05-02-path-properties.md', 
        '7.5.2. Path properties'
      ],
      [
        '07-05-03-paths-and-selections.md', 
        '7.5.3. Paths and selections',
        '7.5.3. 경로와 선택'
      ],
      [
        '07-05-04-transforming-paths.md', 
        '7.5.4. Transforming paths',
        '7.5.4. 경로 변형하기'
      ],
      [
        '07-05-05-stroking-a-path.md', 
        '7.5.5. Stroking a path',
        '7.5.5. 경로 따라 그리기'
      ],
      [
        '07-05-06-paths-and-text.md', 
        '7.5.6. Paths and text',
        '7.5.6. 경로와 텍스트'
      ],
      [
        '07-05-07-paths-and-svg-files.md', 
        '7.5.7. Paths and svg files',
        '7.5.7. 경로와 SVG 파일'
      ],
      [
        '07-06-00-brushes.md', 
        '7.6. Brushes',
        '7.6. 붓'
      ],
      [
        '07-07-adding-new-brushes.md', 
        '7.7. Adding new brushes',
        '7.7. 새 붓 추가하기'
      ],
      [
        '07-08-00-the-gih-dialog-box.md', 
        '7.8. The GIH dialog box',
        '7.8. GIH 대화상자'
      ],
      [
        '07-09-00-changing-brush-size.md', 
        '7.9. Changing brush size',
        '7.9. 붓 크기 바꾸기'
      ],
      [
        '07-09-01-how-to-change-the-size-of-a-brush.md', 
        '7.9.1. How to change the size of a brush',
        '7.9.1. 붓 크기를 바꾸는 방법',
      ],
      [
        '07-09-02-creating-a-brush-quickly.md', 
        '7.9.2. Creating a brush quickly',
        '7.9.2. 새 붓 빨리 만들기'
      ],
      [
        '07-10-00-gradients.md', 
        '7.10. Gradients',
        '7.10. 그라디언트'
      ],
      [
        '07-11-patterns.md', 
        '7.11. Patterns',
        '7.11. 무늬(Pattern)'
      ],
      [
        '07-12-00-palettes.md', 
        '7.12. Palettes',
        '7.12. 팔레트(Palette)'
      ],
      [
        '07-12-01-colormap.md', 
        '7.12.1. Colormap'
      ],
      [
        '07-13-presets.md', 
        '7.13. Presets',
        '7.13. 프리셋(Presets)'
      ],
      [
        '07-14-00-drawing-simple-objects.md', 
        '7.14. Drawing simple objects',
        '7.14. 간단한 물체 그리기'
      ],
      [
        '07-14-01-drawing-a-straight-line.md', 
        '7.14.1. Drawing a straight line',
        '7.14.1. 직선 그리기'
      ],
      [
        '07-14-02-creating-a-basic-shape.md', 
        '7.14.2. Creating a basic shape',
        '7.14.2. 기본 도형 그리기'
      ],
      [
        '08-00-combining-images.md', 
        '8. Combining images',
        '8. 이미지 합치기(Combining Images)'
      ],
      [
        '08-01-00-introduction-to-layers.md', 
        '8.1. Introduction to layers',
        '8.1. 레이어 소개'
      ],
      [
        '08-02-00-layer_modes.md', 
        '8.2. Layer modes',
        '8.2. 레이어 모드'
      ],
      [
        '08-02-01-00-normal-layer-mode.md', 
        '8.2.1. Normal layer mode',
        '8.2.1. 레이어 모드-Normal'
      ],
      [
        '08-02-02-00-lighten-layer-mode.md', 
        '8.2.2. Lighten layer mode',
        '8.2.2. 레이어 모드-Lighten'
      ],
      [
        '08-02-03-00-darken-layer-modes.md', 
        '8.2.3. Darken layer modes',
        '8.2.3. 레이어 모드-어두움(Darken)'
      ],
      [
        '08-02-04-00-contrast-layer-modes.md', 
        '8.2.4. Contrast layer modes',
        '8.2.4. 레이어 모드-대조(Contrast)'
      ],
      [
        '08-02-05-00-inversion-layer-modes.md', 
        '8.2.5. Inversion layer modes',
        '8.2.5. 레이어 모드-반전(Inversion)'
      ],
      [
        '08-02-06-00-hsv-components-layer-modes.md', 
        '8.2.6. HSV components layer modes',
        '8.2.6. 레이어 모드-HSV components'
      ],
      [
        '08-02-07-00-lch-components-layer-modes.md', 
        '8.2.7. LCh components layer modes',
        '8.2.7. 레이어 모드-LCh components'
      ],
      [
        '08-03-00-legacy-layer-modes.md', 
        '8.3. Legacy layer modes',
        '8.3. 레거시 레이어 모드'
      ],
      [
        '08-04-creating-new-layers.md', 
        '8.4. Creating new layers',
        '8.4. 새 레이어 만들기'
      ],
      [
        '08-05-00-layer-groups.md',
        '8.5. Layer groups',
        '8.5. 레이어 그룹'
      ],
      [
        '09-00-text-management.md', 
        '9. Text management'
      ],
      [
        '09-01-00-text-management.md', 
        '9.1. Text management'
      ],
      [
        '09-02-00-text.md', 
        '9.2. Text'
      ],
      [
        '09-02-01-embellishing-text.md', 
        '9.2.1. Embellishing text'
      ],
      [
        '09-02-02-adding-fonts.md', 
        '9.2.2. Adding fonts'
      ],
      [
        '09-02-03-font-problems.md', 
        '9.2.3. Font problems'
      ],
      [
        '10-00-enhancing-photographs.md', 
        '10. Enhancing photographs',
        '10. 사진 편집하기'
      ],
      [
        '10-01-00-working-with-digital-camera-photos.md', 
        '10.1. Working with digital camera photos',
        '10.1. 디지털 카메라로 찍은 사진 편집하기'
      ],
      [
        '11-00-color-management-with-gimp.md', 
        '11. Color management with GIMP',
        '11. GIMP(김프)에서의 색상 관리',
      ],
      [
        '11-01-00-color-management-in-gimp.md', 
        '11.1. Color management in GIMP',
        '11.1. GIMP(김프)에서의 색상 관리'
      ],
      [
        '12-00-enrich-my-gimp.md', 
        '12. Enrich my GIMP',
        '12. 나만의 GIMP(김프) 만들기'
      ],
      [
        '12-01-00-preference-dialog.md', 
        '12.1. Preference dialog',
        '12.1. 기본 설정 대화상자'
      ],
      [
        '12-01-01-introduction.md', 
        '12.1.1. Introduction'
      ],
      [
        '12-01-02-00-system-resources.md', 
        '12.1.2. System Resources',
        '12.1.2. 시스템 자원',
      ],
      [
        '12-01-03-debugging.md', 
        '12.1.3. Debugging'
      ],
      [
        '12-01-04-00-color-management.md', 
        '12.1.4. Color Management',
        '12.1.4. 색상 관리',
      ],
      [
        '12-01-05-image-import-and-export.md', 
        '12.1.5. Image Import and Export',
        '12.1.5. 이미지 불러오기와 내보내기(Image Import and Export)'
      ],
      [
        '12-01-06-experimental-playground.md', 
        '12.1.6. Experimental Playground',
        '12.1.6. 실험 놀이터(Experimental Playground)'
      ],
      [
        '12-01-07-00-tool-options.md', 
        '12.1.7. Tool Options',
        '12.1.7. 도구 옵션(Tool Options)'
      ],
      [
        '12-01-08-default-image-preferences.md', 
        '12.1.8. Default Image Preferences',
        '12.1.8. 기본 이미지 설정(Default Image Preferences)',
      ],
      [
        '12-01-09-default-image-grid.md', 
        '12.1.9. Default Image Grid',
        '12.1.9. 기본 이미지 격자',
      ],
      [
        '12-01-10-00-interface.md', 
        '12.1.10. Interface',
        '12.1.10. 인터페이스(Interface)',
      ],
      [
        '12-01-11-theme.md', 
        '12.1.11. Theme',
        '12.1.11. 테마(Theme)',
      ],
      [
        '12-01-12-icon-theme.md', 
        '12.1.12. Icon Theme'
      ],
      [
        '12-01-13-00-toolbox.md', 
        '12.1.13. Toolbox',
        '12.1.13. 기본 도구상자'
      ],
      [
        '12-01-14-dialog-defaults.md', 
        '12.1.14. Dialog Defaults',
        '12.1.14. 대화상자 기본값(Dialog Defaults)'
      ],
      [
        '12-01-15-00-help_system.md', 
        '12.1.15. Help System',
        '12.1.15. 도움말 시스템(Help System)',
      ],
      [
        '12-01-16-00-display.md', 
        '12.1.16. Display',
        '12.1.16. 화면(Display)',
      ],
      [
        '12-01-17-00-window-management.md', 
        '12.1.17. Window Management',
        '12.1.17. 창 관리(Window Management)',
      ],
      [
        '12-01-18-00-image-window.md', 
        '12.1.18. Image Window',
        '12.1.18. 이미지 창(Image Window)',
      ],
      [
        '12-01-19-image-window-appearance.md', 
        '12.1.19. Image Window Appearance',
        '12.1.19. 이미지 창 모양(Image Window Appearance)',
      ],
      [
        '12-01-20-image-window-title-and-statusbar.md', 
        '12.1.20. Image Window Title and Statusbar',
        '12.1.20. 이미지 창 제목표시줄과 상태표시줄(Image Window Title and Statusbar)',
      ],
      [
        '12-01-21-image-window-snapping-behavior.md', 
        '12.1.21. Image Window Snapping Behavior',
        '12.1.21. 이미지 창 맞추기 동작(Image Window Snapping Behavior)'
      ],
      [
        '12-01-22-input-devices.md', 
        '12.1.22. Input Devices',
        '12.1.22. 입력 장치(Input Devices)'
      ],
      [
        '12-01-23-input-controllers.md', 
        '12.1.23. Input Controllers',
        '12.1.23. 입력 제어기(Input Controllers)',
      ],
      [
        '12-01-24-folders.md', 
        '12.1.24. Folders',
        '12.1.24. 폴더(Folders)',
      ],
      [
        '12-01-25-data-folders.md', 
        '12.1.25. DataFolders',
        '12.1.25. 데이터 폴더(DataFolders)'
      ],
      [
        '12-02-00-grids-and-guides.md', 
        '12.2. Grids and guides',
        '12.2. 격자과 안내선(Grids and guides)'
      ],
      [
        '12-02-01-the-image-grid.md', 
        '12.2.1. The image grid',
        '12.2.1. 이미지 격자(The image grid)'
      ],
      [
        '12-02-02-guides.md', 
        '12.2.2. Guides',
        '12.2.2. 안내선(Guides)'
      ],
      [
        '12-03-rendering-a-grid.md', 
        '12.3. Rendering a grid',
        '12.3. 격자 만들기(Rendering a grid)'
      ],
      [
        '12-04-how-to-set-your-tile-cache.md', 
        '12.4. How to set your tile cache',
        '12.4. 타일 캐쉬 설정하는 방법(How to set your tile cache)'
      ],
      [
        '12-05-creating-shortcuts-to-menu-commands.md', 
        '12.5. Creating shortcuts to menu commands',
        '12.5. 메뉴 단축키 설정하기(Creating shortcuts to menu commands)'
      ],
      [
        '12-06-customize-splash-screen.md', 
        '12.6. Customize splash-screen',
        '12.6. 초기 화면 변경하기(Customize splash-screen)'
      ],
      [
        '13-00-scripting.md', 
        '13. Scripting',
        '13. 스크립팅(Scripting)'
      ],
      [
        '13-01-00-plugins.md', 
        '13.1. Plugins',
        '13.1. 플러그인(Plugins)'
      ],
      [
        '13-02-00-using-script-fu-scripts.md', 
        '13.2. Using script-fu scripts',
        '13.2. Script-Fu 사용하기(Using script-fu scripts)'
      ],
      [
        '13-02-01-script-fu.md', 
        '13.2.1. Script-fu?',
        '13.2.1. Script-fu는 무엇인가요?(Script-fu?)'
      ],
      [
        '13-02-02-installing-script-fu-scripts.md', 
        '13.2.2. Installing script-fu scripts',
        '13.2.2. script-fu 스크립트 설치하기(Installing script-fu scripts)'
      ],
      [
        '13-02-03-do-s-and-don-ts.md', 
        '13.2.3. Do\'s and don\'ts'
      ],
      [
        '13-02-04-different-kinds-of-script-fus.md', 
        '13.2.4. Different kinds of script-fus',
        '13.2.4. 다른 종류의 Script-Fu(Different kinds of Script-Fus)'
      ],
      [
        '13-03-00-a-script-fu-tutorial.md', 
        '13.3. A script-fu tutorial',
        '13.3. Script-Fu 튜토리얼(A Script-Fu tutorial)'
      ],
      [
        '13-03-01-00-getting-acquainted-with-scheme.md', 
        '13.3.1. Getting acquainted with scheme',
        '13.3.1. Scheme에 익숙해지기(Getting acquainted with scheme)'
      ],
      [
        '13-03-02-00-variables-and-functions.md', 
        '13.3.2. Variables and functions',
        '13.3.2. 변수와 함수(Variables and functions)'
      ],
      [
        '13-03-03-00-lists-lists-and-more-lists.md', 
        '13.3.3. Lists, lists and more lists',
        '13.3.3. 더 많은 리스트(Lists, lists and more lists)'
      ],
      [
        '13-03-04-00-your-first-script-fu-script.md', 
        '13.3.4. Your first script-fu script',
        '13.3.4. 첫번째 Script-Fu 스크립트(Your first Script-Fu script)'
      ],
      [
        '13-03-05-00-giving-our-script-some-guts.md', 
        '13.3.5. Giving our script some guts'
      ],
      [
        '13-03-06-00-extending-the-text-box-script.md', 
        '13.3.6. Extending the text box script',
        '13.3.6. 텍스트 박스 스크립트 확장하기(Extending the text box script)'
      ],
      [
        '13-03-07-your-script-and-its-working.md', 
        '13.3.7. Your script and its working',
        '13.3.7. 최종 스크립트 결과 확인하기(Your script and its working)'
      ],
      [
        '14-00-tools.md', 
        '14. 도구(Tools)'
      ],
      [
        '14-01-00-the-toolbox.md', 
        '14.1. 도구상자(The toolbox)'
      ],
      [
        '14-01-01-introduction.md', 
        '14.1.1. 소개(Introduction)',
        '14.1.1. 소개(도구상자)'
      ],
      [
        '14-01-02-tool-icons.md', 
        '14.1.2. 도구 아이콘(Tool icons)',
        '14.1.2. 도구 아이콘(Tool icons)'
      ],
      [
        '14-01-03-00-color-and-indicator-area.md', 
        '14.1.3. 색상과 표시기 영역(Color and indicator area)'
      ],
      [
        '14-01-04-00-tool-options.md',
        '14.1.4. 도구 옵션(Tool options)',
        '14.1.4. 도구 옵션',
      ],
      [
        '14-02-00-selection-tools.md', 
        '14.2. 선택 도구(Selection tools)'
      ],
      [
        '14-02-01-00-common-features.md', 
        '14.2.1. 일반적인 특징(Common features)'
      ],
      [
        '14-02-02-00-rectangle-selection.md', 
        '14.2.2. 사각 선택 도구(Rectangle selection)'
      ],
      [
        '14-02-03-00-ellipse-selection.md', 
        '14.2.3. 타원 선택 도구(Ellipse selection)'
      ],
      [
        '14-02-04-00-free-selection-lasso.md', 
        '14.2.4. 자유 선택 도구(올가미) - Free selection (lasso)',
        '14.2.4. 자유 선택 도구(올가미)'
      ],
      [
        '14-02-05-00-fuzzy-selection-magic-wand.md', 
        '14.2.5. 퍼지 선택(마법 지팡이) - Fuzzy selection(magic wand)'
      ],
      [
        '14-02-06-00-select_by_color.md', 
        '14.2.6. 색상으로 선택(Select by color)'
      ],
      [
        '14-02-07-00-intelligent-scissors.md', 
        '14.2.7. 똑똑한 가위(Intelligent scissors)',
        '14.2.7. 똑똑한 가위'
      ],
      [
        '14-02-08-00-foreground-select.md', 
        '14.2.8. 전경 선택(Foreground select)',
        '14.2.8. 물체 선택 도구'
      ],
      [
        '14-03-00-paint_tools.md', 
        '14.3. 칠하기 도구(Paint tools)'
      ],
      [
        '14-03-01-00-common-features.md', 
        '14.3.1. 일반적인 특징(Common features)'
      ],
      [
        '14-03-02-00-dynamics.md', 
        '14.3.2. 동적 요소(Dynamics)'
      ],
      [
        '14-03-03-00-brush-tools-pencil-paintbrush-airbrush.md', 
        '14.3.3. 붓 도구 (연필, 칠하기붓, 에어브러시) - Brush tools (pencil, paintbrush, airbrush)'
      ],
      [
        '14-03-04-00-bucket_fill.md', 
        '14.3.4. 영역 채우기(Bucket fill)',
        '14.3.4. 영역 채우기'
      ],
      [
        '14-03-05-00-gradient.md', 
        '14.3.5. 그라디언트(Gradient)'
      ],
      [
        '14-03-06-00-pencil.md', 
        '14.3.6. 연필(Pencil)'
      ],
      [
        '14-03-07-00-paintbrush.md', 
        '14.3.7. 그리기 붓(Paintbrush)',
        '14.3.7. 그리기 붓'
      ],
      [
        '14-03-08-00-mypaint-brush.md', 
        '14.3.8. 마이페인트 붓(Mypaint brush)',
        '14.3.8. 마이페인트 붓',
      ],
      [
        '14-03-09-00-eraser.md', 
        '14.3.9. 지우개(Eraser)',
        '14.3.9. 지우개'
      ],
      [
        '14-03-10-00-airbrush.md', 
        '14.3.10. 에어브러시(Airbrush)',
        '14.3.10. 에어브러시',
      ],
      [
        '14-03-11-00-ink.md', 
        '14.3.11. 잉크(Ink)',
        '14.3.11. 잉크'
      ],
      [
        '14-03-12-00-clone.md', 
        '14.3.12. 도장(Clone)',
        '14.3.12. 도장',
      ],
      [
        '14-03-13-00-heal.md', 
        '14.3.13. 복원(Heal)',
        '14.3.13. 복원',
      ],
      [
        '14-03-14-00-perspective-clone.md', 
        '14.3.14. 원근 복제(Perspective clone)',
        '14.3.14. 원근 복제'
      ],
      [
        '14-03-15-00-blur-sharpen.md', 
        '14.3.15. 흐릿하게/선명하게(Blur/sharpen)',
        '14.3.15. 흐릿하게/선명하게'
      ],
      [
        '14-03-16-00-smudge.md', 
        '14.3.16. 문지르기(Smudge)',
        '14.3.16. 문지르기',
      ],
      [
        '14-03-17-00-dodge-burn.md', 
        '14.3.17. 닷지/번(Dodge/Burn)',
        '14.3.17. 닷지/번',
      ],
      [
        '14-04-00-transform-tools.md', 
        '14.4. 변형 도구(Transform tools)'
      ],
      [
        '14-04-01-00-common-features.md', 
        '14.4.1. 일반적인 특징(Common features)'
      ],
      [
        '14-04-02-00-align.md', 
        '14.4.2. 정렬(Align)'
      ],
      [
        '14-04-03-00-move.md', 
        '14.4.3. 이동(Move)'
      ],
      [
        '14-04-04-00-crop.md', 
        '14.4.4. 잘라내기(Crop)',
        '14.4.4. 이미지 잘라내기',
      ],
      [
        '14-04-05-00-rotate.md', 
        '14.4.5. 회전(Rotate)',
        '14.4.5. 회전',
      ],
      [
        '14-04-06-00-scale.md', 
        '14.4.6. 크기 조정(Scale)',
        '14.4.6. 크기 조절',
      ],
      [
        '14-04-07-00-shear.md',
        '14.4.7. 기울이기(Shear)',
        '14.4.7. 기울이기',
      ],
      [
        '14-04-08-00-perspective.md',
        '14.4.8. 원근법(Perspective)',
        '14.4.8. 원근',
      ],
      [
        '14-04-09-00-3d-transform.md', 
        '14.4.9. 3차원 변형(3d transform)'
      ],
      [
        '14-04-10-00-unified-transform.md', 
        '14.4.10. 통합 변형(Unified transform)'
      ],
      [
        '14-04-11-00-handle-transform.md', 
        '14.4.11. 핸들 변형(Handle transform)'
      ],
      [
        '14-04-12-00-flip.md',
        '14.4.12. 뒤집기(Flip)',
        '14.4.12. 뒤집기',
      ],
      [
        '14-04-13-00-the-cage-tool.md', 
        '14.4.13. 골조 변형(The cage tool)'
      ],
      [
        '14-04-14-00-warp-transform.md', 
        '14.4.14. 워프 변형(Warp transform)'
      ],
      [
        '14-05-00-other.md', 
        '14.5. 그 밖의 도구들(Other)'
      ],
      [
        '14-05-01-overview.md', 
        '14.5.1. 개요(Overview)'
      ],
      [
        '14-05-02-00-paths.md', 
        '14.5.2. 경로(Paths)'
      ],
      [
        '14-05-03-00-color_picker.md', 
        '14.5.3. 색상 추출(Color picker)'
      ],
      [
        '14-05-04-00-zoom.md', 
        '14.5.4. 확대/축소(Zoom)'
      ],
      [
        '14-05-05-00-measure.md', 
        '14.5.5. 측정(Measure)'
      ],
      [
        '14-05-06-00-text.md', 
        '14.5.6. 텍스트 도구(Text)'
      ],
      [
        '14-05-07-00-gegl_operation.md', 
        '14.5.7. GEGL 동작(GEGL operation)'
      ],
      [
        '15-00-dialogs.md', 
        '15. 대화상자(Dialogs)'
      ],
      [
        '15-01-dialog-introduction.md', 
        '15.1. 대화상자 소개(Dialog introduction)'
      ],
      [
        '15-02-00-image-structure-related-dialogs.md', 
        '15.2. 이미지 구조 관련 대화상자(Image structure related dialogs)'
      ],
      [
        '15-02-01-00-layers_dialog.md',
        '15.2.1. 레이어 대화상자(Layers dialog)',
        '15.2.1. 레이어 대화상자'
      ],
      [
        '15-02-02-00-channel_dialog.md',
        '15.2.2. 채널 대화상자(Channel dialog)',
        '15.2.2. 채널 대화상자'
      ],
      [
        '15-02-03-00-paths-dialog.md', 
        '15.2.3. 경로 대화상자(Paths dialog)',
        '15.2.3. 경로 대화상자'
      ],
      [
        '15-02-04-00-colormap_dialog.md',
        '15.2.4. 색상표 대화상자(Colormap dialog)',
        '15.2.4. 색상표(Colormap) 대화상자',
      ],
      [
        '15-02-05-00-histogram-dialog.md', 
        '15.2.5. 히스토그램 대화상자(Histogram dialog)'
      ],
      [
        '15-02-06-00-navigation-dialog.md', 
        '15.2.6. 내비게이션 대화상자(Navigation dialog)',
        '15.2.6. 내비게이션 대화상자'
      ],
      [
        '15-02-07-00-undo-history-dialog.md', 
        '15.2.7. 실행취소 이력 대화상자(Undo history dialog)',
        '실행취소 이력 대화상자'
      ],
      [
        '15-03-00-image-content-related-dialogs.md', 
        '15.3. 이미지 콘텐츠 관련 대화상자(Image content related dialogs)'
      ],
      [
        '15-03-01-00-fg-bg-color-dialogs.md', 
        '15.3.1. 전경/배경색 대화상자(FG/BG color dialogs)'
      ],
      [
        '15-03-02-00-brushes_dialog.md', 
        '15.3.2. 붓 대화상자(Brushes dialog)',
        '15.3.2. 붓 대화상자'
      ],
      [
        '15-03-03-00-mypaint-brushes-dialog.md', 
        '15.3.3. 마이페인트 붓 대화상자(Mypaint brushes dialog)'
      ],
      [
        '15-03-04-00-patterns_dialog.md', 
        '15.3.4. 무늬 대화상자(Patterns dialog)',
        '15.3.4. 무늬 대화상자'
      ],
      [
        '15-03-05-00-gradient_dialog.md', 
        '15.3.5. 그라디언트 대화상자(Gradient dialog)'
      ],
      [
        '15-03-06-00-palettes_dialog.md', 
        '15.3.6. 팔레트 대화상자(Palettes dialog)'
      ],
      [
        '15-03-07-00-tagging.md', 
        '15.3.7. 태그(Tagging)'
      ],
      [
        '15-04-00-image-management-related-dialogs.md', 
        '15.4. 이미지 관리 관련 대화상자(Image management related dialogs)'
      ],
      [
        '15-04-01-buffers-dialog.md', 
        '15.4.1. 버퍼 대화상자(Buffers dialog)',
        '15.4.1. 버퍼(Buffers) 대화상자',
      ],
      [
        '15-04-02-00-images-dialog.md', 
        '15.4.2. 이미지 대화상자(Images dialog)',
        '15.4.2. 이미지 대화상자'
      ],
      [
        '15-04-03-00-document-history-dialog.md', 
        '15.4.3. 문서 이력 대화상자(Document history dialog)',
        '15.4.3. 문서 이력 대화상자'
      ],
      [
        '15-04-04-00-templates-dialog.md', 
        '15.4.4. 양식 대화상자(Templates dialog)'
      ],
      [
        '15-05-00-miscellaneous-dialogs.md', 
        '15.5. 그 외의 대화상자(Miscellaneous dialogs)'
      ],
      [
        '15-05-01-00-tool-preset-dialog.md', 
        '15.5.1. 도구 프리셋 대화상자(Tool preset dialog)'
      ],
      [
        '15-05-02-00-tool-preset-editor.md', 
        '15.5.2. 도구 프리셋 편집기(Tool preset editor)'
      ],
      [
        '15-05-03-00-device-status-dialog.md', 
        '15.5.3. 장치 상태 대화상자(Device status dialog)',
        '15.5.3. 장치 상태 대화상자'
      ],
      [
        '15-05-04-00-error-console.md', 
        '15.5.4. 오류 콘솔(Error console)',
        '15.5.4. 오류 콘솔',
      ],
      [
        '15-05-05-dashboard.md', 
        '15.5.5. 상황판(Dashboard)'
      ],
      [
        '15-05-06-00-save-file.md', 
        '15.5.6. 파일 저장하기(Save file)'
      ],
      [
        '15-05-07-00-export-file.md', 
        '15.5.7. Export file',
        '15.5.7. 파일 내보내기(Export file)'
      ],
      [
        '15-05-08-00-sample-points-dialog.md', 
        '15.5.8. 표본점 대화상자(Sample points dialog)'
      ],
      [
        '15-05-09-00-pointer-dialog.md', 
        '15.5.9. 포인터 대화상자(Pointer dialog)'
      ],
      [
        '15-05-10-00-symmetry-painting-dialog.md', 
        '15.5.10. 대칭 그리기 대화상자(Symmetry painting dialog)'
      ],
      [
        '15-05-11-00-keyboard-shortcut-dialog.md', 
        '15.5.11. 키보드 단축키 대화상자(Keyboard shortcut dialog)'
      ],
      [
        '16-00-menus.md', 
        '16. 메뉴(Menus)'
      ],
      [
        '16-01-00-introduction_to_menus.md', 
        '16.1. Introduction to menus'
      ],
      [
        '16-02-00-the-file-menu.md', 
        '16.2. The file menu',
        '16.2. 파일 메뉴(The File Menu)'
      ],
      [
        '16-02-01-overview.md', 
        '16.2.1. 개요(Overview)'
      ],
      [
        '16-02-02-00-new.md', 
        '16.2.2. New'
      ],
      [
        '16-02-03-00-create.md', 
        '16.2.3. 만들기(Create)'
      ],
      [
        '16-02-04-00-file-open.md', 
        '16.2.4. 열기(File open)'
      ],
      [
        '16-02-05-00-open-as-layers.md', 
        '16.2.5. 레이어로 열기(Open as layers)'
      ],
      [
        '16-02-06-00-open-location.md', 
        '16.2.6. 위치 열기(Open Location)'
      ],
      [
        '16-02-07-00-open_recent.md', 
        '16.2.7. 최근 연 파일(Open Recent)'
      ],
      [
        '16-02-08-save.md', 
        '16.2.8. 저장(Save)'
      ],
      [
        '16-02-09-00-save-as.md', 
        '16.2.9. 다른 이름으로 저장(Save As)'
      ],
      [
        '16-02-10-00-save-a-copy.md', 
        '16.2.10. 사본 저장(Save a Copy)'
      ],
      [
        '16-02-11-00-revert.md', 
        '16.2.11. 원래대로(Revert)'
      ],
      [
        '16-02-12-export-and-overwrite.md', 
        '16.2.12. 이미지 내보내기와 덮어쓰기(Export… and Overwrite…)'
      ],
      [
        '16-02-13-00-export-as.md', 
        '16.2.13. 다른 이름으로 이미지 내보내기(Export as)'
      ],
      [
        '16-02-14-00-create-template.md', 
        '16.2.14. 템플릿 만들기…(Create Template…)'
      ],
      [
        '16-02-15-00-print.md', 
        '16.2.15. 인쇄(Print)'
      ],
      [
        '16-02-16-send-by-email.md', 
        '16.2.16. 이메일로 보내기(Send by Email)'
      ],
      [
        '16-02-17-copy-image-location.md', 
        '16.2.17. 이미지 위치 복사하기(Copy Image Location)'
      ],
      [
        '16-02-18-show-in-file-manager.md', 
        '16.2.18. 파일 관리자에 표시(Show in File Manager)'
      ],
      [
        '16-02-19-00-close.md', 
        '16.2.19. 이미지 닫기(Close)'
      ],
      [
        '16-02-20-00-close-all.md', 
        '16.2.20. 모든 이미지 창 닫기(Close All)'
      ],
      [
        '16-02-21-00-quit.md', 
        '16.2.21. 끝내기(Quit)'
      ],
      [
        '16-03-00-the-edit-menu.md', 
        '16.3. 편집 메뉴(The edit menu)'
      ],
      [
        '16-03-01-edit-menu-entries.md', 
        '16.3.1. 편집 메뉴 항목("Edit" menu entries)'
      ],
      [
        '16-03-02-00-undo.md', 
        '16.3.2. 실행 취소(Undo)'
      ],
      [
        '16-03-03-00-redo.md', 
        '16.3.3. Redo'
      ],
      [
        '16-03-04-00-undo-history.md', 
        '16.3.4. 실행취소 이력(Undo History)'
      ],
      [
        '16-03-05-00-cut.md', 
        '16.3.5. 잘라내기(Cut)'
      ],
      [
        '16-03-06-00-copy.md', 
        '16.3.6. 복사(Copy)'
      ],
      [
        '16-03-07-00-copy-visible.md', 
        '16.3.7. 보이는 부분 복사(Copy Visible)'
      ],
      [
        '16-03-08-00-paste.md', 
        '16.3.8. 붙여넣기(Paste)',
        '16.3.8. 붙여넣기',
      ],
      [
        '16-03-09-00-paste_into_selection.md', 
        '16.3.9. 선택 영역에 붙여넣기(Paste into selection)',
        '16.3.9. 선택 영역에 붙여넣기',
      ],
      [
        '16-03-10-00-paste_in_place.md', 
        '16.3.10. 지정한 좌표에 붙여넣기(Paste In Place)'
      ],
      [
        '16-03-11-00-paste_into_selection_in_place.md', 
        '16.3.11. 지정한 좌표의 선택영역에 붙여넣기(Paste into selection in place)'
      ],
      [
        '16-03-12-00-paste_as.md', 
        '16.3.12. 붙여넣기(고급) - Paste as'
      ],
      [
        '16-03-13-00-buffer.md', 
        '16.3.13. 버퍼(Buffer)'
      ],
      [
        '16-03-14-00-clear.md', 
        '16.3.14. 지우기(Clear)'
      ],
      [
        '16-03-15-00-fill-with-fg-color.md', 
        '16.3.15. 전경색으로 채우기(Fill with FG Color)'
      ],
      [
        '16-03-16-00-fill-with-bg-color.md', 
        '16.3.16. 배경색으로 채우기(Fill with BG Color)'
      ],
      [
        '16-03-17-00-fill-with-pattern.md', 
        '16.3.17. 무늬로 채우기(Fill with pattern)'
      ],
      [
        '16-03-18-00-fill-selection-outline.md', 
        '16.3.18. 선택 영역 채우기(Fill Selection Outline)'
      ],
      [
        '16-03-19-00-fill_path.md', 
        '16.3.19. 경로 채우기(Fill path)'
      ],
      [
        '16-03-20-00-stroke-selection.md', 
        '16.3.20. 선택 영역 따라 그리기(Stroke selection)'
      ],
      [
        '16-03-21-00-stroke-path.md', 
        '16.3.21. 경로 따라 그리기(Stroke Path)'
      ],
      [
        '16-03-22-00-preference.md', 
        '16.3.22. 기본 설정(Preferences)'
      ],
      [
        '16-03-23-00-input-devices.md', 
        '16.3.23. 입력장치(Input Devices)'
      ],
      [
        '16-03-24-00-keyboard-shortcuts.md', 
        '16.3.24. 키보드 단축키(Keyboard Shortcuts)'
      ],
      [
        '16-03-25-00-modules.md', 
        '16.3.25. 모듈(Modules)'
      ],
      [
        '16-03-26-00-units.md', 
        '16.3.26. 단위(Units)'
      ],
      [
        '16-04-00-the-select-menu.md', 
        '16.4. 선택 메뉴(The select menu)',
        '16.4. 선택 메뉴',
      ],
      [
        '16-04-01-introduction-to-the-select-menu.md', 
        '16.4.1. 선택 메뉴 소개(Introduction to the select menu)'
      ],
      [
        '16-04-02-00-select-all.md', 
        '16.4.2. 모두 선택(Select all)'
      ],
      [
        '16-04-03-00-none.md',
        '16.4.3. 없음(None)',
        '16.4.3. 없음(None)',
      ],
      [
        '16-04-04-00-invert.md', 
        '16.4.4. 반전(Invert)',
        '16.4.4. 선택 반전',
      ],
      [
        '16-04-05-00-float.md',
        '16.4.5. Float',
        '16.4.5. 선택 띄우기(Float)'
      ],
      [
        '16-04-06-00-by_color.md', 
        '16.4.6. 색상 선택(By Color)',
        '16.4.6. 색상 선택',
      ],
      [
        '16-04-07-00-from_path.md', 
        '16.4.7. 경로에서(From path)'
      ],
      [
        '16-04-08-00-selection_editor.md', 
        '16.4.8. 선택 영역 편집기(Selection Editor)'
      ],
      [
        '16-04-09-00-feather.md', 
        '16.4.9. 페더(Feather)'
      ],
      [
        '16-04-10-00-sharpen.md', 
        '16.4.10. 또렷하게(Sharpen)'
      ],
      [
        '16-04-11-00-shrink.md', 
        '16.4.11. 줄이기(Shrink)'
      ],
      [
        '16-04-12-00-grow.md', 
        '16.4.12. 늘이기(Grow)'
      ],
      [
        '16-04-13-00-border.md', 
        '16.4.13. 테두리(Border)'
      ],
      [
        '16-04-14-remove-holes.md', 
        '16.4.14. 구멍 제거(Remove Holes)'
      ],
      [
        '16-04-15-00-distort.md', 
        '16.4.15. 왜곡(Distort)'
      ],
      [
        '16-04-16-00-rounded-rectangle.md', 
        '16.4.16. 사각형 모서리 둥글게 하기(Rounded rectangle)'
      ],
      [
        '16-04-17-toggle-quickmask.md', 
        '16.4.17. 퀵 마스크 전환(Toggle Quick Mask)'
      ],
      [
        '16-04-18-save-to-channel.md', 
        '16.4.18. 채널로 저장(Save to channel)',
        '선택을 채널로(마스크)'
      ],
      [
        '16-04-19-to-path.md', 
        '16.4.19. 경로로(To path)'
      ],
      [
        '16-05-00-the-view-menu.md', 
        '16.5. 보기 메뉴(The view menu)',
        '16.5. 보기 메뉴',
      ],
      [
        '16-05-01-introduction-to-the-view-menu.md', 
        '16.5.1. 보기 메뉴 소개(Introduction to the "View" menu)'
      ],
      [
        '16-05-02-00-new-view.md', 
        '16.5.2. 새 창으로 보기(New View)'
      ],
      [
        '16-05-03-00-show_all.md', 
        '16.5.3. 모두 보기(Show All)'
      ],
      [
        '16-05-04-00-dot-for-dot.md', 
        '16.5.4. 점대점(Dot for dot)',
        '16.5.4. 점 대 점'
      ],
      [
        '16-05-05-00-zoom.md', 
        '16.5.5. 확대(Zoom)'
      ],
      [
        '16-05-06-00-flip-rotate.md', 
        '16.5.6. 뒤집기와 회전 (0°)(Flip & Rotate (0°))'
      ],
      [
        '16-05-07-center-image-in-window.md', 
        '16.5.7. 이미지를 창의 가운데로 옮김(Center Image in Window)'
      ],
      [
        '16-05-08-00-shrink-wrap.md', 
        '16.5.8. Shrink wrap'
      ],
      [
        '16-05-09-00-full-screen.md', 
        '16.5.9. 전체화면(Fullscreen)'
      ],
      [
        '16-05-10-navigation-window.md', 
        '16.5.10. 내비게이션 창(Navigation Window)'
      ],
      [
        '16-05-11-00-display-filters.md', 
        '16.5.11. 필터 보이기(Display Filters)'
      ],
      [
        '16-05-12-color-management.md', 
        '16.5.12. 색상 관리(Color Management)'
      ],
      [
        '16-05-13-show-selection.md', 
        '16.5.13. 선택 영역 보이기(Show selection)'
      ],
      [
        '16-05-14-show-layer-boundary.md', 
        '16.5.14. 레이어 경계 보이기(Show Layer Boundary)',
        '16.5.14. 레이어 경계 보이기',
      ],
      [
        '16-05-15-show-canvas-boundary.md', 
        '16.5.15. 캔버스 경계 보이기(Show Canvas Boundary)'
      ],
      [
        '16-05-16-show-guides.md', 
        '16.5.16. 안내선 보이기(Show Guides)'
      ],
      [
        '16-05-17-show-grid.md', 
        '16.5.17. 격자 보이기(Show Grid)'
      ],
      [
        '16-05-18-show_sample_points.md', 
        '16.5.18. 표본점 보이기(Show sample points)'
      ],
      [
        '16-05-19-snap-to-guides.md', 
        '16.5.19. 안내선에 맞추기(Snap to Guides)'
      ],
      [
        '16-05-20-snap-to-grid.md', 
        '16.5.20. 격자에 맞추기(Snap to grid)'
      ],
      [
        '16-05-21-snap_to_canvas_edges.md', 
        '16.5.21. 캔버스 경계에 맞추기(Snap to Canvas Edges)'
      ],
      [
        '16-05-22-snap-to-active-path.md', 
        '16.5.22. 활성 경로에 맞추기(Snap to Active Path)'
      ],
      [
        '16-05-23-00-padding-color.md', 
        '16.5.23. 색 채우기(Padding Color)'
      ],
      [
        '16-05-24-show_menubar.md', 
        '16.5.24. 메뉴 표시줄 보이기(Show Menubar)'
      ],
      [
        '16-05-25-show_rulers.md', 
        '16.5.25. 눈금자 보이기(Show Rulers)'
      ],
      [
        '16-05-26-show_scrollbars.md', 
        '16.5.26. 스크롤 막대 표시(Show Scrollbars)'
      ],
      [
        '16-05-27-show-statusbar.md', 
        '16.5.27. 상태표시줄 보이기(Show statusbar)'
      ],
      [
        '16-06-00-the-image-menu.md', 
        '16.6. 이미지 메뉴(The "Image" menu)'
      ],
      [
        '16-06-01-overview.md', 
        '16.6.1. 이미지 메뉴 개요(Overview)'
      ],
      [
        '16-06-02-duplicate.md', 
        '16.6.2. 복제(Duplicate)'
      ],
      [
        '16-06-03-mode.md', 
        '16.6.3. 모드(Mode)',
        '16.6.3. 이미지 모드',
      ],
      [
        '16-06-04-rgb-mode.md', 
        '16.6.4. RGB 모드(RGB mode)'
      ],
      [
        '16-06-05-00-grayscale-mode.md', 
        '16.6.5. 그레이스케일 모드(Grayscale mode)'
      ],
      [
        '16-06-06-00-indexed-mode.md', 
        '16.6.6. Indexed mode',
        '16.6.6. 인덱스 모드(Indexed mode)',
      ],
      [
        '16-06-07-00-precision.md', 
        '16.6.7. 정밀도(Precision)'
      ],
      [
        '16-06-08-color-management.md', 
        '16.6.8. 색상 관리(Color management)'
      ],
      [
        '16-06-09-00-enable-color-management.md', 
        '16.6.9. 색상 관리 활성화(Enable color management)'
      ],
      [
        '16-06-10-00-assign-color-profile.md', 
        '16.6.10. 색상 프로파일 할당(Assign color profile)'
      ],
      [
        '16-06-11-00-convert-to-color-profile.md', 
        '16.6.11. Convert to color profile'
      ],
      [
        '16-06-12-00-discard-color-profile.md', 
        '16.6.12. 색상 프로파일 버리기(Discard color profile)'
      ],
      [
        '16-06-13-00-save-color-profile-to-file.md', 
        '16.6.13. 색상 프로파일 파일로 저장하기(Save color profile to file)'
      ],
      [
        '16-06-14-transform.md', 
        '16.6.14. 변형(Transform)'
      ],
      [
        '16-06-15-flip-horizontally-flip-vertically.md', 
        '16.6.15. 수평 뒤집기, 수직 뒤집기(Flip Horizontally, Flip Vertically)'
      ],
      [
        '16-06-16-00-rotation.md', 
        '16.6.16. 회전(Rotation)'
      ],
      [
        '16-06-17-00-canvas-size.md', 
        '16.6.17. 캔버스 크기(Canvas size)'
      ],
      [
        '16-06-18-fit-canvas-to-layers.md', 
        '16.6.18. 캔버스 크기를 레이어에 맞춥니다(Fit Canvas to Layers)'
      ],
      [
        '16-06-19-fit-canvas-to-selection.md', 
        '16.6.19. 선택 영역에 캔버스 크기 맞추기(Fit Canvas to Selection)'
      ],
      [
        '16-06-20-00-print-size.md', 
        '16.6.20. 인쇄 크기(Print Size)'
      ],
      [
        '16-06-21-00-scale-image.md', 
        '16.6.21. 이미지 크기조정(Scale Image)'
      ],
      [
        '16-06-22-00-crop-image.md', 
        '16.6.22. 이미지 잘라내기(Crop image)'
      ],
      [
        '16-06-23-slice-using-guides.md', 
        '16.6.23. 안내선으로 잘라내기(Slice Using Guides)'
      ],
      [
        '16-06-24-zealous-crop.md', 
        '16.6.24. 지능적으로 잘라내기(Zealous Crop)'
      ],
      [
        '16-06-25-00-merge_visible_layers.md', 
        '16.6.25. 보이는 레이어 합치기(Merge visible layers)'
      ],
      [
        '16-06-26-flatten_image.md', 
        '16.6.26. 이미지 평탄화하기(Flatten image)'
      ],
      [
        '16-06-27-00-align-visible-layers.md', 
        '16.6.27. 보이는 레이어 정렬…(Align visible layers…)'
      ],
      [
        '16-06-28-guides.md', 
        '16.6.28. 안내선(Guides)'
      ],
      [
        '16-06-29-new-guide.md', 
        '16.6.29. 새 안내선(New guide)'
      ],
      [
        '16-06-30-new-guide-by-percent.md', 
        '16.6.30. 새 안내선 (퍼센트) - New guide (by percent)'
      ],
      [
        '16-06-31-new-guide-from-selection.md', 
        '16.6.31. 선택으로 새 안내선 만들기(New guide from selection)'
      ],
      [
        '16-06-32-remove-all-guides.md', 
        '16.6.32. 모든 안내선 지우기(Remove all guides)'
      ],
      [
        '16-06-33-00-configure-grid.md', 
        '16.6.33. 격자 설정…(Configure grid…)'
      ],
      [
        '16-06-34-00-image-properties.md', 
        '16.6.34. 이미지 성질(Image properties)'
      ],
      [
        '16-06-35-00-metadata.md', 
        '16.6.35. 메타데이터(Metadata)'
      ],
      [
        '16-06-35-01-metadata-viewer.md', 
        '16.6.35.1. 메타데이터 조회(Metadata viewer)'
      ],
      [
        '16-06-35-02-metadata-editor.md', 
        '16.6.35.2. 메타데이터 수정(Metadata editor)'
      ],
      [
        '16-07-00-the-layer-menu.md', 
        '16.7. 레이어 메뉴(The "Layer" menu)'
      ],
      [
        '16-07-01-introduction-to-the-layer-menu.md', 
        '16.7.1. 레이어 메뉴 소개(Introduction to the "Layer" menu)'
      ],
      [
        '16-07-02-00-new_layer.md',
        '16.7.2. 새 레이어(New layer)',
        '16.7.2. 새 레이어',
      ],
      [
        '16-07-03-new_from_visible.md', 
        '16.7.3. 보이는 영역에서 새로 만들기(New from visible)'
      ],
      [
        '16-07-04-new-layer-group.md', 
        '16.7.4. 새 레이어 그룹(New Layer Group)'
      ],
      [
        '16-07-05-duplicate-layer.md', 
        '16.7.5. 레이어 복제(Duplicate Layer)'
      ],
      [
        '16-07-06-anchor-layer.md',
        '16.7.6. 레이어 고정하기(Anchor layer)',
        '16.7.6. 레이어 고정',
      ],
      [
        '16-07-07-merge-down.md', 
        '16.7.7. 아래로 합치기(Merge Down)'
      ],
      [
        '16-07-08-merge-layer-group.md', 
        '16.7.8. 레이어 그룹 합치기(Merge layer group)'
      ],
      [
        '16-07-09-delete-layer.md', 
        '16.7.9. 레이어 삭제(Delete Layer)'
      ],
      [
        '16-07-10-the-text-commands-of-the-layer-menu.md', 
        '16.7.10. 레이어 메뉴의 텍스트 명령(The text commands of the layer menu)'
      ],
      [
        '16-07-11-discard-text-information.md', 
        '16.7.11. 텍스트 정보 버리기(Discard text information)'
      ],
      [
        '16-07-12-stack-submenu.md', 
        '16.7.12. 스택 하위메뉴(Stack submenu)'
      ],
      [
        '16-07-13-select-previous-layer.md', 
        '16.7.13. 이전 레이어 선택(Select previous layer)'
      ],
      [
        '16-07-14-select-next-layer.md', 
        '16.7.14. 다음 레이어 선택(Select next layer)'
      ],
      [
        '16-07-15-select-top-layer.md', 
        '16.7.15. 맨 위 레이어 선택(Select top layer)'
      ],
      [
        '16-07-16-select-bottom-layer.md', 
        '16.7.16. 바닥 레이어 선택(Select bottom layer)'
      ],
      [
        '16-07-17-raise-layer.md', 
        '16.7.17. 레이어 올리기(Raise layer)'
      ],
      [
        '16-07-18-lower-layer.md', 
        '16.7.18. 레이어 내리기(Lower layer)'
      ],
      [
        '16-07-19-layer-to-top.md', 
        '16.7.19. 레이어를 꼭대기로(Layer to top)'
      ],
      [
        '16-07-20-layer-to-bottom.md', 
        '16.7.20. 레이어를 바닥으로(Layer to bottom)'
      ],
      [
        '16-07-21-the-reverse-layer-order-command.md', 
        '16.7.21. 레이어 순서 뒤집기(The reverse layer order command)'
      ],
      [
        '16-07-22-the-mask-submenu.md', 
        '16.7.22. 마스크 하위메뉴(The mask submenu)'
      ],
      [
        '16-07-23-00-add_layer_mask.md', 
        '16.7.23. 레이어 마스크 추가하기(Add layer mask)'
      ],
      [
        '16-07-24-apply_layer_mask.md', 
        '16.7.24. 레이어 마스크 적용하기(Apply layer mask)'
      ],
      [
        '16-07-25-delete_layer_mask.md', 
        '16.7.25. 레이어 마스크 삭제하기(Delete layer mask)'
      ],
      [
        '16-07-26-show_layer_mask.md', 
        '16.7.26. 레이어 마스크 보여주기(Show layer mask)'
      ],
      [
        '16-07-27-edit_layer_mask.md', 
        '16.7.27. 레이어 마스크 수정하기(Edit layer mask)'
      ],
      [
        '16-07-28-disable_layer_mask.md', 
        '16.7.28. 레이어 마스크 비활성화하기(Disable layer mask)'
      ],
      [
        '16-07-29-mask_to_selection.md', 
        '16.7.29. 마스크를 선택으로 바꾸기(Mask to selection)'
      ],
      [
        '16-07-30-add-layer-mask-to-selection.md', 
        '16.7.30. 선택 영역에 추가(Add layer mask to selection)'
      ],
      [
        '16-07-31-subtract-layer-mask-from-selection.md', 
        '16.7.31. 선택 영역에서 빼기(Subtract layer mask from selection)'
      ],
      [
        '16-07-32-intersect-layer-mask-with-selection.md', 
        '16.7.32. 선택 영역과 겹치기(Intersect layer mask with selection)'
      ],
      [
        '16-07-33-the-transparency-submenu-of-the-layer-menu.md', 
        '16.7.33. 레이어 메뉴의 투명 하위메뉴(The transparency submenu of the layer menu)'
      ],
      [
        '16-07-34-add_alpha_channel.md', 
        '16.7.34. 알파 채널 추가(Add alpha channel)'
      ],
      [
        '16-07-35-remove_alpha_channel.md', 
        '16.7.35. 알파 채널 제거(Remove alpha channel)'
      ],
      [
        '16-07-36-color-to-alpha.md', 
        '16.7.36. 색상을 알파로(Color to alpha)'
      ],
      [
        '16-07-37-semi-flatten.md', 
        '16.7.37. Semi flatten'
      ],
      [
        '16-07-38-00-threshold-alpha.md', 
        '16.7.38. 임계 알파(Threshold alpha)'
      ],
      [
        '16-07-39-alpha-to-selection.md', 
        '16.7.39. 알파를 선택 영역으로(Alpha to selection)'
      ],
      [
        '16-07-40-add-alpha-channel-to-selection.md', 
        '16.7.40. 선택에 추가(Add alpha channel to selection)'
      ],
      [
        '16-07-41-subtract-from-selection.md', 
        '16.7.41. 선택 영역에서 빼기(Subtract from selection)'
      ],
      [
        '16-07-42-intersect-alpha-channel-with-selection.md', 
        '16.7.42. 선택 영역과 겹치기(Intersect alpha channel with selection)'
      ],
      [
        '16-07-43-the-transform-submenu.md', 
        '16.7.43. 변형 메뉴(The transform submenu)'
      ],
      [
        '16-07-44-flip-horizontally.md', 
        '16.7.44. 수평 뒤집기(Flip horizontally)'
      ],
      [
        '16-07-45-flip-vertically.md', 
        '16.7.45. 수직 뒤집기(Flip vertically)'
      ],
      [
        '16-07-46-rotate-90-clockwise.md', 
        '16.7.46. 시계방향으로 90도 회전(Rotate 90° clockwise)'
      ],
      [
        '16-07-47-rotate-90-counter-clockwise.md', 
        '16.7.47. 반시계방향으로 90도 회전(Rotate 90° counter-clockwise)'
      ],
      [
        '16-07-48-rotate-180.md', 
        '16.7.48. 180도 회전(Rotate 180°)'
      ],
      [
        '16-07-49-arbitrary-rotation.md', 
        '16.7.49. 임의 각도 회전(Arbitrary rotation)'
      ],
      [
        '16-07-50-00-offset.md', 
        '16.7.50. 오프셋(Offset)'
      ],
      [
        '16-07-51-00-layer_boundary_size.md', 
        '16.7.51. 레이어 경계 크기(Layer boundary size)'
      ],
      [
        '16-07-52-layer_to_image_size.md', 
        '16.7.52. 레이어를 이미지 크기로(Layer to image size)'
      ],
      [
        '16-07-53-00-scale_layer.md', 
        '16.7.53. 레이어 크기조정(Scale layer)'
      ],
      [
        '16-07-54-00-crop-layer.md', 
        '16.7.54. 레이어 잘라내기(Crop layer)'
      ],
      [
        '16-08-00-the-colors-menu.md', 
        '16.8. 색 메뉴(The colors menu)'
      ],
      [
        '16-08-01-00-introduction-to-the-colors-menu.md', 
        '16.8.1. Introduction to the colors menu'
      ],
      [
        '16-08-02-00-color-balance.md', 
        '16.8.2. 색상 균형(Color balance)'
      ],
      [
        '16-08-03-00-color-temperature.md', 
        '16.8.3. 색 온도(Color temperature)'
      ],
      [
        '16-08-04-00-hue-chroma.md', 
        '16.8.4. 색상-채도(Chroma)(Hue-Chroma)'
      ],
      [
        '16-08-05-00-hue-saturation.md', 
        '16.8.5. 색상-채도(Saturation)'
      ],
      [
        '16-08-06-00-saturation.md', 
        '16.8.6. 채도(Saturation)'
      ],
      [
        '16-08-07-00-exposure.md', 
        '16.8.7. 노출(Exposure)'
      ],
      [
        '16-08-08-00-shadows-highlights.md', 
        '16.8.8. 그림자-강조(Shadows-Highlights)'
      ],
      [
        '16-08-09-00-brightness-contrast.md', 
        '16.8.9. 명도-대비(Brightness-Contrast)'
      ],
      [
        '16-08-10-00-levels.md', 
        '16.8.10. 레벨(Levels)'
      ],
      [
        '16-08-11-00-curves.md', 
        '16.8.11. 커브(Curves)'
      ],
      [
        '16-08-12-invert.md', 
        '16.8.12. 반전(Invert)'
      ],
      [
        '16-08-13-linear-invert.md', 
        '16.8.13. 선형 반전(Linear Invert)'
      ],
      [
        '16-08-14-value-invert.md', 
        '16.8.14. 명도 반전(Value Invert)'
      ],
      [
        '16-08-15-the-auto-submenu.md', 
        '16.8.15. 자동 하위 메뉴(The auto submenu)'
      ],
      [
        '16-08-16-equalize.md', 
        '16.8.16. 평준화(Equalize)'
      ],
      [
        '16-08-17-white-balance.md', 
        '16.8.17. 화이트 밸런스(White Balance)'
      ],
      [
        '16-08-18-00-stretch-contrast.md', 
        '16.8.18. 대비 증가…(Stretch Contrast…)'
      ],
      [
        '16-08-19-stretch-contrast-hsv.md', 
        '16.8.19. HSV 대비 증가(Stretch Contrast HSV)'
      ],
      [
        '16-08-20-color-enhance.md', 
        '16.8.20. 색상 보강(Color Enhance)'
      ],
      [
        '16-08-21-color-enhance-legacy.md', 
        '16.8.21. 색상 보강(레거시) - Color Enhance (legacy)'
      ],
      [
        '16-08-22-the-components-submenu.md', 
        '16.8.22. 성분 하위 메뉴(The "Components" submenu)'
      ],
      [
        '16-08-23-00-channel-mixer.md', 
        '16.8.23. Channel mixer',
        '16.8.23. 채널 혼합(Channel Mixer)'
      ],
      [
        '16-08-24-00-extract-component.md', 
        '16.8.24. 요소 추출…(Extract Component…)'
      ],
      [
        '16-08-25-00-mono-mixer.md', 
        '16.8.25. Mono Mixer…'
      ],
      [
        '16-08-26-00-compose.md', 
        '16.8.26. 구성…(Compose…)'
      ],
      [
        '16-08-27-00-decompose.md', 
        '16.8.27. 분해…(Decompose…)'
      ],
      [
        '16-08-28-recompose.md', 
        '16.8.28. 재구성(Recompose)'
      ],
      [
        '16-08-29-the-desaturate-submenu.md', 
        '16.8.29. 무채화 하위메뉴(The "Desaturate" submenu)'
      ],
      [
        '16-08-30-00-color-to-gray.md', 
        '16.8.30. 흑백 전환(Color to Gray…)'
      ],
      [
        '16-08-31-00-desaturate.md', 
        '16.8.31. 무채화…(Desaturate…)'
      ],
      [
        '16-08-32-mono-mixer.md', 
        '16.8.32. Mono Mixer…'
      ],
      [
        '16-08-33-00-sepia.md', 
        '16.8.33. 세피아…(Sepia…)'
      ],
      [
        '16-08-34-the-map-submenu.md', 
        '16.8.34. 지도 하위 메뉴(The "Map" submenu)'
      ],
      [
        '16-08-35-rearrange-colormap.md', 
        '16.8.35. 색상표 재배치(Rearrange colormap)'
      ],
      [
        '16-08-36-set-colormap.md', 
        '16.8.36. 색상표 설정…(Set Colormap…)'
      ],
      [
        '16-08-37-00-alien-map.md', 
        '16.8.37. 에일리언 맵…(Alien Map…)'
      ],
      [
        '16-08-38-00-color-exchange.md', 
        '16.8.38. 색상 교환…(Color Exchange…)'
      ],
      [
        '16-08-39-00-rotate-colors.md', 
        '16.8.39. 색 전환…(Rotate Colors…)'
      ],
      [
        '16-08-40-gradient-map.md', 
        '16.8.40. 그라디언트 맵(Gradient Map)'
      ],
      [
        '16-08-41-palette-map.md', 
        '16.8.41. 팔레트 맵(Palette Map)'
      ],
      [
        '16-08-42-00-sample-colorize.md', 
        '16.8.42. 표본 색상화…(Sample Colorize…)'
      ],
      [
        '16-08-43-the_tone_mapping_submenu.md', 
        '16.8.43. 톤 매핑 하위 메뉴(The "Tone Mapping" submenu)'
      ],
      [
        '16-08-44-00-fattal_et_al_2002.md', 
        '16.8.44. Fattal et al. 2002'
      ],
      [
        '16-08-45-00-mantiuk_2006.md', 
        '16.8.45. 만티크 2006(Mantiuk 2006)'
      ],
      [
        '16-08-46-00-reinhard_2005.md', 
        '16.8.46. 라인하르트 2005(Reinhard 2005)'
      ],
      [
        '16-08-47-00-stress.md', 
        '16.8.47. Stress'
      ],
      [
        '16-08-48-00-retinex.md', 
        '16.8.48. 레티넥스(Retinex)'
      ],
      [
        '16-08-49-the-info-submenu.md', 
        '16.8.49. 정보 하위 메뉴(The "Info" submenu)'
      ],
      [
        '16-08-50-histogram.md', 
        '16.8.50. 히스토그램(Histogram)'
      ],
      [
        '16-08-51-00-border-average.md', 
        '16.8.51. 테두리 평균(Border Average)'
      ],
      [
        '16-08-52-colorcube-analysis.md', 
        '16.8.52. 색상 큐브 분석(Colorcube Analysis)'
      ],
      [
        '16-08-53-00-export-histogram.md', 
        '16.8.53. 히스토그램 내보내기(Export Histogram)'
      ],
      [
        '16-08-54-smooth-palette.md', 
        '16.8.54. 부드러운 팔레트(Smooth Palette)'
      ],
      [
        '16-08-55-00-threshold.md', 
        '16.8.55. 임계(Threshold)'
      ],
      [
        '16-08-56-00-colorize.md', 
        '16.8.56. 색상화(Colorize)'
      ],
      [
        '16-08-57-00-posterize.md', 
        '16.8.57. Posterize'
      ],
      [
        '16-08-58-00-color-to-alpha.md', 
        '16.8.58. 색상을 알파로(Color to alpha)'
      ],
      [
        '16-08-59-00-dither.md', 
        '16.8.59. 디더링(Dither)'
      ],
      [
        '16-08-60-00-rgb-clip.md', 
        '16.8.60. RGB 클립(RGB Clip)'
      ],
      [
        '16-08-61-hot.md', 
        '16.8.61. 뜨거움(Hot)'
      ],
      [
        '16-09-00-the-tools-menu.md', 
        '16.9. The "Tools" menu'
      ],
      [
        '16-09-01-introduction-to-the-tools-menu.md', 
        '16.9.1. Introduction to the "Tools" menu'
      ],
      [
        '16-10-00-the-filters-menu.md', 
        '16.10. The "Filters" menu'
      ],
      [
        '16-10-01-introduction-to-the-filters-menu.md', 
        '16.10.1. Introduction to the "Filters" menu'
      ],
      [
        '16-10-02-repeat-last.md', 
        '16.10.2. Repeat last'
      ],
      [
        '16-10-03-re-show-last.md', 
        '16.10.3. Re show last'
      ],
      [
        '16-10-04-reset-all-filters.md', 
        '16.10.4. Reset all filters'
      ],
      [
        '16-10-05-the-python-fu-submenu.md', 
        '16.10.5. The "Python-Fu" submenu'
      ],
      [
        '16-10-06-the-script-fu-submenu.md', 
        '16.10.6. The "Script-Fu" submenu'
      ],
      [
        '16-11-windows-menu.md', 
        '16.11. "Windows" menu'
      ],
      [
        '16-12-00-the-help-menu.md', 
        '16.12. The "Help" menu'
      ],
      [
        '16-12-01-introduction-to-the-help-menu.md', 
        '16.12.1. Introduction to the "Help" menu'
      ],
      [
        '16-12-02-help.md', 
        '16.12.2. Help'
      ],
      [
        '16-12-03-context-help.md', 
        '16.12.3. Context help'
      ],
      [
        '16-12-04-tip-of-the-day.md', 
        '16.12.4. Tip of the day'
      ],
      [
        '16-12-05-about.md', 
        '16.12.5. About'
      ],
      [
        '16-12-06-search-and-run-a-command.md', 
        '16.12.6. Search and run a command'
      ],
      [
        '16-12-07-plug-in-browser.md', 
        '16.12.7. Plug-in browser'
      ],
      [
        '16-12-08-the-procedure-browser.md', 
        '16.12.8. The procedure browser'
      ],
      [
        '16-12-09-bug-reports-and-feature-requests.md', 
        '16.12.9. Bug reports and feature requests'
      ],
      [
        '16-12-10-gimp-online.md', 
        '16.12.10. GIMP online'
      ],
      [
        '16-12-11-user-manual.md', 
        '16.12.11. User manual'
      ],
      [
        '17-00-filters.md', 
        '17. Filters'
      ],
      [
        '17-01-introduction.md', 
        '17.1. Introduction'
      ],
      [
        '17-02-00-common-features.md', 
        '17.2. 공통 특징(Common features)'
      ],
      [
        '17-03-00-blur-filters.md', 
        '17.3. 흐림 필터(Blur filters)'
      ],
      [
        '17-03-01-introduction.md', 
        '17.3.1. Introduction'
      ],
      [
        '17-03-02-focus-blur.md', 
        '17.3.2. Focus blur'
      ],
      [
        '17-03-03-gaussian-blur.md', 
        '17.3.3. Gaussian blur'
      ],
      [
        '17-03-04-lens-blur.md', 
        '17.3.4. Lens blur'
      ],
      [
        '17-03-05-mean-curvature-blur.md', 
        '17.3.5. Mean curvature blur'
      ],
      [
        '17-03-06-median-blur.md', 
        '17.3.6. Median blur'
      ],
      [
        '17-03-07-pixelize.md', 
        '17.3.7. Pixelize'
      ],
      [
        '17-03-08-selective-gaussian-blur.md', 
        '17.3.8. Selective gaussian blur'
      ],
      [
        '17-03-09-variable-blur.md', 
        '17.3.9. Variable blur'
      ],
      [
        '17-03-10-circular-motion-blur.md', 
        '17.3.10. Circular motion blur'
      ],
      [
        '17-03-11-linear-motion-blur.md', 
        '17.3.11. Linear motion blur'
      ],
      [
        '17-03-12-zoom-motion-blur.md', 
        '17.3.12. Zoom motion blur'
      ],
      [
        '17-03-13-tileable-blur.md', 
        '17.3.13. Tileable blur'
      ],
      [
        '17-04-00-enhance-filters.md', 
        '17.4. 강화 필터(Enhance filters)'
      ],
      [
        '17-04-01-introduction.md', 
        '17.4.1. Introduction'
      ],
      [
        '17-04-02-antialias.md', 
        '17.4.2. Antialias'
      ],
      [
        '17-04-03-deinterlace.md', 
        '17.4.3. Deinterlace'
      ],
      [
        '17-04-04-high-pass.md', 
        '17.4.4. High pass'
      ],
      [
        '17-04-05-noise-reduction.md', 
        '17.4.5. Noise reduction'
      ],
      [
        '17-04-06-red-eye-removal.md', 
        '17.4.6. Red eye removal'
      ],
      [
        '17-04-07-symmetric-nearest-neighbor.md', 
        '17.4.7. Symmetric nearest neighbor'
      ],
      [
        '17-04-08-sharpen-unsharp-mask.md', 
        '17.4.8. 선명하게 - Sharpen (unsharp mask)'
      ],
      [
        '17-04-09-despeckle.md', 
        '17.4.9. Despeckle'
      ],
      [
        '17-04-10-destripe.md', 
        '17.4.10. Destripe'
      ],
      [
        '17-04-11-nl-filter.md', 
        '17.4.11. NL filter'
      ],
      [
        '17-04-12-wavelet-decompose.md', 
        '17.4.12. Wavelet decompose'
      ],
      [
        '17-05-00-distort-filters.md', 
        '17.5. Distort filters'
      ],
      [
        '17-05-01-introduction.md', 
        '17.5.1. Introduction'
      ],
      [
        '17-05-02-apply-lens.md', 
        '17.5.2. Apply lens'
      ],
      [
        '17-05-03-emboss.md', 
        '17.5.3. Emboss'
      ],
      [
        '17-05-04-engrave.md', 
        '17.5.4. Engrave'
      ],
      [
        '17-05-05-lens-distortion.md', 
        '17.5.5. Lens distortion'
      ],
      [
        '17-05-06-kaleidoscope.md', 
        '17.5.6. Kaleidoscope'
      ],
      [
        '17-05-07-mosaic.md', 
        '17.5.7. Mosaic'
      ],
      [
        '17-05-08-newsprint.md', 
        '17.5.8. Newsprint'
      ],
      [
        '17-05-09-polar-coordinates.md', 
        '17.5.9. Polar coordinates'
      ],
      [
        '17-05-10-ripple.md', 
        '17.5.10. Ripple'
      ],
      [
        '17-05-11-shift.md', 
        '17.5.11. Shift'
      ],
      [
        '17-05-12-spherize.md', 
        '17.5.12. Spherize'
      ],
      [
        '17-05-13-value-propagate.md', 
        '17.5.13. Value propagate'
      ],
      [
        '17-05-14-video.md', 
        '17.5.14. Video'
      ],
      [
        '17-05-15-waves.md', 
        '17.5.15. Waves'
      ],
      [
        '17-05-16-whirl-and-pinch.md', 
        '17.5.16. Whirl and pinch'
      ],
      [
        '17-05-17-wind.md', 
        '17.5.17. Wind'
      ],
      [
        '17-05-18-curve-bend.md', 
        '17.5.18. Curve bend'
      ],
      [
        '17-05-19-emboss-legacy.md', 
        '17.5.19. Emboss (legacy)'
      ],
      [
        '17-05-20-page-curl.md', 
        '17.5.20. Page curl'
      ],
      [
        '17-06-00-light-and-shadow-filters.md', 
        '17.6. Light and shadow filters'
      ],
      [
        '17-06-01-introduction.md', 
        '17.6.1. Introduction'
      ],
      [
        '17-06-02-bloom.md', 
        '17.6.2. Bloom'
      ],
      [
        '17-06-03-supernova.md', 
        '17.6.3. Supernova'
      ],
      [
        '17-06-04-lens-flare.md', 
        '17.6.4. Lens flare'
      ],
      [
        '17-06-05-gradient-flare.md', 
        '17.6.5. Gradient flare'
      ],
      [
        '17-06-06-lighting-effects.md', 
        '17.6.6. Lighting effects'
      ],
      [
        '17-06-07-sparkle.md', 
        '17.6.7. Sparkle'
      ],
      [
        '17-06-08-drop-shadow.md', 
        '17.6.8. Drop shadow'
      ],
      [
        '17-06-09-long-shadow.md', 
        '17.6.9. Long shadow'
      ],
      [
        '17-06-10-vignette.md', 
        '17.6.10. Vignette'
      ],
      [
        '17-06-11-drop-shadow-legacy.md', 
        '17.6.11. Drop shadow (legacy)'
      ],
      [
        '17-06-12-perspective.md', 
        '17.6.12. Perspective'
      ],
      [
        '17-06-13-xach-effect.md', 
        '17.6.13. Xach-Effect'
      ],
      [
        '17-07-00-noise-filters.md', 
        '17.7. Noise filters'
      ],
      [
        '17-07-01-introduction.md', 
        '17.7.1. Introduction'
      ],
      [
        '17-07-02-cie-ich-noise.md', 
        '17.7.2. CIE Ich noise'
      ],
      [
        '17-07-03-hsv-noise.md', 
        '17.7.3. HSV noise'
      ],
      [
        '17-07-04-hurl.md', 
        '17.7.4. Hurl'
      ],
      [
        '17-07-05-pick.md', 
        '17.7.5. Pick'
      ],
      [
        '17-07-06-rgb-noise.md', 
        '17.7.6. RGB noise'
      ],
      [
        '17-07-07-slur.md', 
        '17.7.7. Slur'
      ],
      [
        '17-07-08-spread.md', 
        '17.7.8. Spread'
      ],
      [
        '17-08-00-edge-detect-filters.md', 
        '17.8. Edge detect filters'
      ],
      [
        '17-08-01-introduction.md', 
        '17.8.1. Introduction'
      ],
      [
        '17-08-02-difference-of-gaussians.md', 
        '17.8.2. Difference of gaussians'
      ],
      [
        '17-08-03-edge.md', 
        '17.8.3. Edge'
      ],
      [
        '17-08-04-laplace.md', 
        '17.8.4. Laplace'
      ],
      [
        '17-08-05-neon.md', 
        '17.8.5. Neon'
      ],
      [
        '17-08-06-sobel.md', 
        '17.8.6. Sobel'
      ],
      [
        '17-08-07-image-gradient.md', 
        '17.8.7. Image gradient'
      ],
      [
        '17-09-00-generic-filters.md', 
        '17.9. Generic filters'
      ],
      [
        '17-09-01-introduction.md', 
        '17.9.1. Introduction'
      ],
      [
        '17-09-02-convolution-matrix.md', 
        '17.9.2. Convolution matrix'
      ],
      [
        '17-09-03-distance-map.md', 
        '17.9.3. Distance map'
      ],
      [
        '17-09-04-gegl-graph.md', 
        '17.9.4. GEGL graph'
      ],
      [
        '17-09-05-normal-map.md', 
        '17.9.5. Normal map'
      ],
      [
        '17-09-06-dilate.md', 
        '17.9.6. Dilate'
      ],
      [
        '17-09-07-erode.md', 
        '17.9.7. Erode'
      ],
      [
        '17-10-00-combine-filters.md', 
        '17.10. Combine filters'
      ],
      [
        '17-10-01-introduction.md', 
        '17.10.1. Introduction'
      ],
      [
        '17-10-02-depth-merge.md', 
        '17.10.2. Depth merge'
      ],
      [
        '17-10-03-filmstrip.md', 
        '17.10.3. Filmstrip'
      ],
      [
        '17-11-00-artstic-filters.md', 
        '17.11. Artstic filters'
      ],
      [
        '17-11-01-introduction.md', 
        '17.11.1. Introduction'
      ],
      [
        '17-11-02-apply-canvas.md', 
        '17.11.2. Apply canvas'
      ],
      [
        '17-11-03-cartoon.md', 
        '17.11.3. Cartoon'
      ],
      [
        '17-11-04-cubism.md', 
        '17.11.4. Cubism'
      ],
      [
        '17-11-05-glass-tile.md', 
        '17.11.5. Glass tile'
      ],
      [
        '17-11-06-oilify.md', 
        '17.11.6. Oilify'
      ],
      [
        '17-11-07-photocopy.md', 
        '17.11.7. Photocopy'
      ],
      [
        '17-11-08-simple-linear-iterative-clustering-slic.md', 
        '17.11.8. Simple linear iterative clustering (SLIC)'
      ],
      [
        '17-11-09-softglow.md', 
        '17.11.9. Softglow'
      ],
      [
        '17-11-10-waterpixels.md', 
        '17.11.10. Waterpixels'
      ],
      [
        '17-11-11-cartoon-legacy.md', 
        '17.11.11. Cartoon (legacy)'
      ],
      [
        '17-11-12-clothify.md', 
        '17.11.12. Clothify'
      ],
      [
        '17-11-13-gimpressionist.md', 
        '17.11.13. Gimpressionist'
      ],
      [
        '17-11-14-oilify-legacy.md', 
        '17.11.14. Oilify (legacy)'
      ],
      [
        '17-11-15-predator.md', 
        '17.11.15. Predator'
      ],
      [
        '17-11-16-softglow-legacy.md', 
        '17.11.16. Softglow (legacy)'
      ],
      [
        '17-11-17-van-gogh-lic.md', 
        '17.11.17. Van gogh (LIC)'
      ],
      [
        '17-11-18-weave.md', 
        '17.11.18. Weave'
      ],
      [
        '17-12-00-decor-filters.md', 
        '17.12. Decor filters'
      ],
      [
        '17-12-01-introduction.md', 
        '17.12.1. Introduction'
      ],
      [
        '17-12-02-add-bevel.md', 
        '17.12.2. Add bevel'
      ],
      [
        '17-12-03-add-border.md', 
        '17.12.3. Add border'
      ],
      [
        '17-12-04-coffee-stain.md', 
        '17.12.4. Coffee stain'
      ],
      [
        '17-12-05-fog.md', 
        '17.12.5. Fog'
      ],
      [
        '17-12-06-fuzzy-border.md', 
        '17.12.6. Fuzzy border'
      ],
      [
        '17-12-07-old-photo.md', 
        '17.12.7. Old photo'
      ],
      [
        '17-12-08-round-corners.md', 
        '17.12.8. Round corners'
      ],
      [
        '17-12-09-slide.md', 
        '17.12.9. Slide'
      ],
      [
        '17-12-10-stencil-carve.md', 
        '17.12.10. Stencil carve'
      ],
      [
        '17-12-11-stencil-chrome.md', 
        '17.12.11. Stencil chrome'
      ],
      [
        '17-13-00-map-filters.md', 
        '17.13. Map filters'
      ],
      [
        '17-13-01-introduction.md', 
        '17.13.1. Introduction'
      ],
      [
        '17-13-02-bump-map.md', 
        '17.13.2. Bump map'
      ],
      [
        '17-13-03-displace.md', 
        '17.13.3. Displace'
      ],
      [
        '17-13-04-fractal-trace.md', 
        '17.13.4. Fractal trace'
      ],
      [
        '17-13-05-illusion.md', 
        '17.13.5. Illusion'
      ],
      [
        '17-13-06-little-planet.md', 
        '17.13.6. Little planet'
      ],
      [
        '17-13-07-panorama-projection.md', 
        '17.13.7. Panorama projection'
      ],
      [
        '17-13-08-recursive-transform.md', 
        '17.13.8. Recursive transform'
      ],
      [
        '17-13-09-paper-tile.md', 
        '17.13.9. Paper tile'
      ],
      [
        '17-13-10-tile-seamless.md', 
        '17.13.10. Tile seamless'
      ],
      [
        '17-13-11-fractal-trace-legacy.md', 
        '17.13.11. Fractal trace (legacy)'
      ],
      [
        '17-13-12-map-object.md', 
        '17.13.12. Map object'
      ],
      [
        '17-13-13-tile.md', 
        '17.13.13. Tile'
      ],
      [
        '17-14-00-rendering-filters.md', 
        '17.14. Rendering filters'
      ],
      [
        '17-14-01-introduction.md', 
        '17.14.1. Introduction'
      ],
      [
        '17-14-02-flame.md', 
        '17.14.2. Flame'
      ],
      [
        '17-14-03-fractal-explorer.md', 
        '17.14.3. 프랙탈 탐색기(Fractal explorer)'
      ],
      [
        '17-14-04-ifs-fractal.md', 
        '17.14.4. IFS fractal'
      ],
      [
        '17-14-05-cell-noise.md', 
        '17.14.5. Cell noise'
      ],
      [
        '17-14-06-perlin-noise.md', 
        '17.14.6. Perlin noise'
      ],
      [
        '17-14-07-plasma.md', 
        '17.14.7. Plasma'
      ],
      [
        '17-14-08-simplex-noise.md', 
        '17.14.8. Simplex noise'
      ],
      [
        '17-14-09-solid-noise.md', 
        '17.14.9. Solid noise'
      ],
      [
        '17-14-10-difference-clouds.md', 
        '17.14.10. Difference clouds'
      ],
      [
        '17-14-11-bayer-matrix.md', 
        '17.14.11. Bayer matrix'
      ],
      [
        '17-14-12-checkerboard.md', 
        '17.14.12. Checkerboard'
      ],
      [
        '17-14-13-diffraction-patterns.md', 
        '17.14.13. Diffraction patterns'
      ],
      [
        '17-14-14-grid.md', 
        '17.14.14. Grid'
      ],
      [
        '17-14-15-linear-sinusoid.md', 
        '17.14.15. Linear sinusoid'
      ],
      [
        '17-14-16-maze.md', 
        '17.14.16. Maze'
      ],
      [
        '17-14-17-sinus.md', 
        '17.14.17. Sinus'
      ],
      [
        '17-14-18-spiral.md', 
        '17.14.18. Spiral'
      ],
      [
        '17-14-19-checkerboard-legacy.md', 
        '17.14.19. Checkerboard (legacy)'
      ],
      [
        '17-14-20-cml-explorer.md', 
        '17.14.20. CML explorer'
      ],
      [
        '17-14-21-grid-legacy.md', 
        '17.14.21. Grid (legacy)'
      ],
      [
        '17-14-22-jigsaw.md', 
        '17.14.22. Jigsaw'
      ],
      [
        '17-14-23-qbist.md', 
        '17.14.23. Qbist'
      ],
      [
        '17-14-24-spyrogimp.md', 
        '17.14.24. Spyrogimp'
      ],
      [
        '17-14-25-circuit.md', 
        '17.14.25. Circuit'
      ],
      [
        '17-14-26-gfig.md', 
        '17.14.26. Gfig'
      ],
      [
        '17-14-27-lava.md', 
        '17.14.27. Lava'
      ],
      [
        '17-14-28-line-nova.md', 
        '17.14.28. Line nova'
      ],
      [
        '17-14-29-sphere-designer.md', 
        '17.14.29. Sphere designer'
      ],
      [
        '17-15-00-web-filters.md', 
        '17.15. Web filters'
      ],
      [
        '17-15-01-introduction.md', 
        '17.15.1. Introduction'
      ],
      [
        '17-15-02-imagemap.md', 
        '17.15.2. Imagemap'
      ],
      [
        '17-15-03-semi-flatten.md', 
        '17.15.3. Semi-Flatten'
      ],
      [
        '17-15-04-slice.md', 
        '17.15.4. Slice'
      ],
      [
        '17-16-00-animation-filters.md', 
        '17.16. Animation filters'
      ],
      [
        '17-16-01-introduction.md', 
        '17.16.1. Introduction'
      ],
      [
        '17-16-02-blend.md', 
        '17.16.2. Blend'
      ],
      [
        '17-16-03-burn-in.md', 
        '17.16.3. Burn-in'
      ],
      [
        '17-16-04-rippling.md', 
        '17.16.4. Rippling'
      ],
      [
        '17-16-05-spinning-globe.md', 
        '17.16.5. Spinning globe'
      ],
      [
        '17-16-06-waves.md', 
        '17.16.6. Waves'
      ],
      [
        '17-16-07-optimize.md', 
        '17.16.7. Optimize'
      ],
      [
        '17-16-08-playback.md', 
        '17.16.8. Playback'
      ],
      [
        '18-00-keys-and-mouse-reference.md', 
        '18. Keys and mouse reference'
      ],
      [
        '18-01-file-key-reference-for-the-file-menu.md', 
        '18.1. File - key reference for the file menu'
      ],
      [
        '18-02-edit-key-reference-for-the-edit-menu.md', 
        '18.2. Edit - key reference for the edit menu'
      ],
      [
        '18-03-select-key-reference-for-the-select-menu.md', 
        '18.3. Select - key reference for the select menu'
      ],
      [
        '18-04-view-key-reference-for-the-view-menu.md', 
        '18.4. View - key reference for the view menu'
      ],
      [
        '18-05-image-key-reference-for-the-image-menu.md', 
        '18.5. Image - key reference for the image menu'
      ],
      [
        '18-06-layers-key-reference-for-the-layer-menu.md', 
        '18.6. Layers - key reference for the layer menu'
      ],
      [
        '18-07-tools-key-reference-for-the-tools-menu.md', 
        '18.7. Tools - key reference for the tools menu'
      ],
      [
        '18-08-filters-key-reference-for-the-filters-menu.md', 
        '18.8. Filters - key reference for the filters menu'
      ],
      [
        '18-09-windows-key-reference-for-the-windows-menu.md', 
        '18.9. Windows - key reference for the windows menu'
      ],
      [
        '18-10-help-key-reference-for-the-help-menu.md', 
        '18.10. Help - key reference for the help menu'
      ],
      [
        '18-11-zoom-tool-key-reference-for-the-zoom-tool.md', 
        '18.11. Zoom tool - key reference for the zoom tool'
      ],
      [
        '19-glossary.md', 
        '19. Glossary'
      ],
      [
        '20-00-tone-mapping-and-shadow-recovery-using-gimp-colors-exposure.md', 
        '20. Tone mapping and shadow recovery using GIMP\'s "Colors/Exposure"'
      ],
      [
        '20-01-high-bit-depth-gimp-floating-point-colors-exposure-much-better-curves-for-lightening-the-shadows-and-midtones-of-an-image-without-blowing-the-highlights.md', 
        '20.1. High bit depth GIMP\'s floating point "Colors/Exposure"" much better curves for lightening the shadows and midtones of an image without blowing the highlights'
      ],
      [
        '20-02-a-step-by-step-example-showing-how-to-recover-shadow-information-using-high-bit-depth-gimp-floating-point-colors-exposure.md', 
        '20.2. A step-by-step example showing how to recover shadow information using high bit depth GIMP floating point colors exposure'
      ],
      [
        '20-03-use-notes.md', 
        '20.3. Use notes'
      ],
      [
        '20-04-conclusion.md', 
        '20.4. Conclusion'
      ],
    ],
  ),
}


