# 6.1.2. 파일 형식
이미지 파일을 저장하고 내보내는 여러가지 명령이 있습니다. 이 장에서 설명하는 명령들은 [16.2. 파일 메뉴(The File Menu)](./16-02-00-the-file-menu.md)에서 다루고 있습니다.
...
이미지를 내보내더라도 이미지 원본은 바뀌지 않습니다. 더 자세한 내용은 [15.5.7. 파일 내보내기(Export file)](./15-05-07-00-export-file.md)에서 확인해보세요.
...
> 🗒️ 참고
>
> GIMP(김프)에서는 이미지가 원본과 다르게 수정된 뒤에 저장되지 않은 이미지의 상태를 [`dirty`](./19-glossaryx-dirty_image.md)라고 표현합니다. 이미지를 닫을 때(아마도 GIMP를 끌 때), 이미지의 상태가 `dirty`라면 경고 메시지 대화상자(그림 90.4.42.a101)를 볼 수 있습니다. 또한 [타이틀 바](./03-02-04-01-title-bar.md)에 표시된 파일 이름의 맨 앞에 `*`이 붙어 있는 것(그림 90.5.1.a131)을 확인할 수 있습니다.
>
