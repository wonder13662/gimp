...
[`선형 조명(Linear light)`](./19-glossaryx-linear_light.md)와 `가시영역 감마 sRGB(Perceptual gamma sRGB)` 중에서 하나를 선택할 수 있습니다.

8비트 정수 정밀도(기본값)를 사용할 경우에는 `가시영역 감마 sRGB(Perceptual gamma sRGB)`이며, 32비트 [부동소수점](./19-glossaryx-floating_point.md) 정밀도를 사용할 경우에는 [`선형 조명(Linear light)`](./19-glossaryx-linear_light.md)을 선택합니다. 
