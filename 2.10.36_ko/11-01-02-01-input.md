...
이미지를 이미지에 포함된 `색상 프로필(Color Profile)`으로 열 때, GIMP(김프)는 이미지 파일을 `RGB 작업 색 공간`으로 바꾸도록 해줍니다. `sRGB`가 기본값이며, 모든 작업이 `sRGB` 색 공간에서 완료되는 것이 좋습니다. 원한다면, 이미지에 첨부된 `색상 프로필(Color Profile)`을 작업 색 공간으로 유지할 수도 있습니다.
