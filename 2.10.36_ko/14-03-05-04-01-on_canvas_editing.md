...
[그라디언트 편집 대화상자](./15-03-05-00-gradient_dialog.md)의 모든 기능을 캔버스에서도 사용할 수 있습니다.
...
`시작 끝점(Start endpoint)`과 `종단 끝점(End endpoint)` 중 하나를 클릭, 드래그해서 원하는 곳으로 옮깁니다. 이 때 마우스 포인터에 이동 십자(moving cross)가 추가됩니다. 아래 영상에서 `시작 끝점(Start endpoint)`에 마우스 포인터를 올리는 시점에 이동 십자(moving cross) 아이콘이 추가되는 모습을 확인할 수 있습니다.
