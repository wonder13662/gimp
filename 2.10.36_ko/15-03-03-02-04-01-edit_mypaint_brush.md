# 15.3.3.2.4.1. 마이페인트 붓 편집(Edit Mypaint brush)

<a id="90-04-17-a101-02-04-01"></a>

#### [그림 90.4.17.a101.2.4.1. '마이페인트 붓' 대화상자: 격자 모드 → `대화상자 아래쪽 버튼들` → `마이페인트 붓 편집` (Windows) (우리말)](./90-04-0017-mypaint_brushes.md#90-04-17-a101-02-04-01)
<img width="200" height="255" alt="90-04-17-a101-02-04-01" src="https://github.com/user-attachments/assets/d0d07ad5-4392-4aa9-9d47-84f3e9fe50f9" />

`마이페인트 붓 편집(Edit Mypaing brush)` 버튼은 GIMP(김프)가 마이페인트 붓을 편집하는 것을 지원하지 않게 되면서 더 이상 사용할 수 없게 되었습니다.

<a comment="TODO 이 버튼 제거해달라는 요청을 올려야 함."></a>

***

## 다른 페이지로 가기

[➡️ 다음: 15.3.3.2.4.2. 마이페인트 붓 복제(Duplicate brush)](./15-03-03-02-04-02-duplicate_brush.md)

[⬅️ 이전: 15.3.3.2.4. 대화상자 아래쪽 버튼들과 슬라이더(Buttons at the bottom)](./15-03-03-02-04-00-buttons_at_the_bottom.md)

[⬆️ 위: 15.3.3.2.4. 대화상자 아래쪽 버튼들과 슬라이더(Buttons at the bottom)](./15-03-03-02-04-00-buttons_at_the_bottom.md)

[⬆️ 위: 15.3.3.2. 마이페인트 붓 대화상자 사용하기(Using the Mypaint brushes dialog)](./15-03-03-02-00-using_the_mypaint_brushes_dialog.md)

[⬆️ 위: 15.3.3. 마이페인트 붓 대화상자(Mypaint brushes dialog)](./15-03-03-00-mypaint-brushes-dialog.md)

[⬆️ 위: 15.3. 이미지 콘텐츠 관련 대화상자(Image content related dialogs)](./15-03-00-image-content-related-dialogs.md)

[⬆️ 위: 15. 대화상자(Dialogs)](./15-00-dialogs.md)

[🏠 홈](./00-home.md)

[원문](https://docs.gimp.org/2.10/ko/gimp-mypaint-brush-dialog.html#gimp-mypaint-brush-dialog-buttons)