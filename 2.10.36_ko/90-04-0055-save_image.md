# 90.4.55. 이미지 저장(Save Image)

<a id="90-04-55-a101"></a>

#### 그림 90.4.55.a101. `이미지 저장` 대화상자 (Windows) (우리말)
<img width="746" height="552" alt="90-04-55-a101" src="https://github.com/wonder13662/gimp/assets/15767104/4998ac9f-01a4-469f-a414-4c2e48f37525" />

[16.2.8. 저장(Save) - 그림 90.4.55.a101](./16-02-08-save.md#90-04-55-a101)

[16.2.9. 다른 이름으로 저장(Save As) - 그림 90.4.55.a101](./16-02-09-00-save-as.md#90-04-55-a101)

[16.2.9.2. 이미지 저장 대화상자(The "Save Image" dialog) - 그림 90.4.55.a101](./16-02-09-02-the_save_image_dialog.md#90-04-55-a101)

<a id="90-04-55-a102"></a>

#### 그림 90.4.55.a102. `이미지 저장` 대화상자 (Windows) (영어)
<img width="746" height="555" alt="90-04-55-a102" src="https://github.com/wonder13662/gimp/assets/15767104/62afbf4d-078d-46d0-92a6-c8ef80007933" />

<a id="90-04-55-a103"></a>

#### 그림 90.4.55.a103. `이미지 저장` 대화상자 (Mac) (우리말)
<img width="792" height="650" alt="90-04-55-a103" src="https://github.com/wonder13662/gimp/assets/15767104/4a840e18-f025-4cc5-8f79-455ea0694eed" />

<a id="90-04-55-a104"></a>

#### 그림 90.4.55.a104. `이미지 저장` 대화상자 (Mac) (영어)
<img width="792" height="650" alt="90-04-55-a104" src="https://github.com/wonder13662/gimp/assets/15767104/83bf433a-861b-4331-83c5-c8690f7e202a" />

<a id="90-04-55-a111"></a>

#### 영상 90.4.55.a111. `이미지 저장` 대화상자: Ctrl(⌘) + `S`로 작업중인 이미지 저장하기 (Windows) (우리말)
<video controls="controls" width="640" height="360" src="https://github.com/user-attachments/assets/8eb10a4c-bd99-493c-95c6-cdfb617be57a"></video>

[15.5.6.1. 파일 저장하기 대화상자 활성화하기(Activate the Dialog) - 영상 90.4.55.a111](./15-05-06-01-activate_the_dialog.md#90-04-55-a111)

<a id="90-04-55-a121"></a>

#### 영상 90.4.55.a121. `이미지 저장` 대화상자: Ctrl(⌘) + Shift(⇧) + `S`로 작업중인 이미지를 다른 이름으로 저장하기 (Windows) (우리말)
<video controls="controls" width="640" height="360" src="https://github.com/user-attachments/assets/2a0bb465-fe32-42d0-ad7e-46a566c7ef7c"></video>

[15.5.6.1. 파일 저장하기 대화상자 활성화하기(Activate the Dialog) - 영상 90.4.55.a121](./15-05-06-01-activate_the_dialog.md#90-04-55-a121)

<a id="90-04-55-a131"></a>

#### 그림 90.4.55.a131. `이미지 저장` 대화상자 → `이름` 입력창 (Windows) (우리말)
<img width="746" height="552" alt="90-04-55-a131" src="https://github.com/user-attachments/assets/d7ccd7f4-07cf-4729-a6f8-1e1ef7e0b4ba" />

[15.5.6.2. 이미지 저장하기 대화상자(The save image dialog) - 그림 90.4.55.a131](./15-05-06-02-the_save_image_dialog.md#90-04-55-a131)

<a id="90-04-55-a141"></a>

#### 그림 90.4.55.a141. `이미지 저장` 대화상자 → `폴더에 저장` (Windows) (우리말)
<img width="746" height="552" alt="90-04-55-a141" src="https://github.com/user-attachments/assets/f9d4f1c3-a0b7-4804-9eb6-1d034bd1793c" />

[15.5.6.2. 이미지 저장하기 대화상자(The save image dialog) - 그림 90.4.55.a141](./15-05-06-02-the_save_image_dialog.md#90-04-55-a141)

<a id="90-04-55-a141-01"></a>

#### 영상 90.4.55.a141.1. `이미지 저장` 대화상자 → `폴더에 저장`: 폴더 바꾸기 (Windows) (우리말)
<video controls="controls" width="640" height="360" src="https://github.com/user-attachments/assets/a0091560-164d-416e-b46f-de9f45d6dadf"></video>

[15.5.6.2. 이미지 저장하기 대화상자(The save image dialog) - 영상 90.4.55.a141.1](./15-05-06-02-the_save_image_dialog.md#90-04-55-a141-01)

<a id="90-04-55-a151"></a>

#### 그림 90.4.55.a151. `이미지 저장` 대화상자 → `폴더 만들기` (Windows) (우리말)
<img width="746" height="552" alt="90-04-55-a151" src="https://github.com/user-attachments/assets/a6a37b0d-6968-444d-ab67-cbde6cf3fca7" />

[15.5.6.2. 이미지 저장하기 대화상자(The save image dialog) - 그림 90.4.55.a151](./15-05-06-02-the_save_image_dialog.md#90-04-55-a151)

<a id="90-04-55-a151-01"></a>

#### 영상 90.4.55.a151.1. `이미지 저장` 대화상자 → `폴더 만들기` (Windows) (우리말)
<video controls="controls" width="640" height="360" src="https://github.com/user-attachments/assets/e600e93f-9a04-4304-87e7-6f0af3e8d72b"></video>

[15.5.6.2. 이미지 저장하기 대화상자(The save image dialog) - 영상 90.4.55.a151.1](./15-05-06-02-the_save_image_dialog.md#90-04-55-a151-01)

<a id="90-04-55-a161"></a>

#### 그림 90.4.55.a161. `이미지 저장` 대화상자 → `파일 유형 선택` (Windows) (우리말)
<img width="741" height="769" alt="90-04-55-a161" src="https://github.com/user-attachments/assets/432dbd52-8400-4c31-ab89-9033dd7df9b4" />

[15.5.6.2. 이미지 저장하기 대화상자(The save image dialog) - 그림 90.4.55.a161](./15-05-06-02-the_save_image_dialog.md#90-04-55-a161)

<a id="90-04-55-a161-01"></a>

#### 영상 90.4.55.a161.1. `이미지 저장` 대화상자 → `파일 유형 선택` (Windows) (우리말)
<video controls="controls" width="640" height="360" src="https://github.com/user-attachments/assets/71fc565d-9570-4a76-8ad4-b9b0fd5f7c54"></video>

[15.5.6.2. 이미지 저장하기 대화상자(The save image dialog) - 영상 90.4.55.a161.1](./15-05-06-02-the_save_image_dialog.md#90-04-55-a161-01)

<a id="90-04-55-a171"></a>

#### 그림 90.4.55.a171. `이미지 저장` 대화상자 (Windows) (우리말)
<img width="707" height="619" alt="90-04-55-a171" src="https://github.com/user-attachments/assets/c2b742e3-7f56-4a10-993d-1ecd8df19e8c" />

<a id="90-04-55-a181"></a>

#### 그림 90.4.55.a181. `이미지 저장` 대화상자 → `주요 디렉토리` (Windows) (우리말)
<img width="707" height="619" alt="90-04-55-a181" src="https://github.com/user-attachments/assets/5eee50c6-7e90-4b5c-a18b-fbf3041b54f3" />

[16.2.9.2.1. 주요 디렉토리(Main directories) - 그림 90.4.55.a181](./16-02-09-02-01-main_directories.md#90-04-55-a181)

<a id="90-04-55-a182"></a>

#### 그림 90.4.55.a182. `이미지 저장` 대화상자 → `책갈피 디렉토리` (Windows) (우리말)
<img width="707" height="619" alt="90-04-55-a182" src="https://github.com/user-attachments/assets/640be839-04b4-4fb0-8fbb-4516b927c887" />

[16.2.9.2.2. 책갈피 디렉토리(Bookmarks) - 그림 90.4.55.a182](./16-02-09-02-02-bookmarks.md#90-04-55-a182)

<a id="90-04-55-a183"></a>

#### 그림 90.4.55.a183. `이미지 저장` 대화상자 → `현재 디렉토리` (Windows) (우리말)
<img width="707" height="619" alt="90-04-55-a183" src="https://github.com/user-attachments/assets/be1a128b-93c2-4551-8403-9d2c6b8782df" />

[16.2.9.2.3. 현재 디렉토리(Current Directory) - 그림 90.4.55.a183](./16-02-09-02-03-current_directory.md#90-04-55-a183)

<a id="90-04-55-a184"></a>

#### 그림 90.4.55.a184. `이미지 저장` 대화상자 → `미리보기` (Windows) (우리말)
<img width="707" height="619" alt="90-04-55-a184" src="https://github.com/user-attachments/assets/da526225-1b31-42dd-b3c2-4fa13ef9887e" />

[16.2.9.2.4. 미리보기(Preview) - 그림 90.4.55.a184](./16-02-09-02-04-preview.md#90-04-55-a184)

<a id="90-04-55-a185"></a>

#### 그림 90.4.55.a185. `이미지 저장` 대화상자 → `이름` (Windows) (우리말)
<img width="707" height="619" alt="90-04-55-a185" src="https://github.com/user-attachments/assets/3b485ed7-0555-4533-9c43-185b5b060475" />

[16.2.9.2.5. 이름(Name) - 그림 90.4.55.a185](./16-02-09-02-05-name.md#90-04-55-a185)

<a id="90-04-55-a186"></a>

#### 그림 90.4.55.a186. `이미지 저장` 대화상자 → `현재 디렉토리 경로` (Windows) (우리말)
<img width="707" height="619" alt="90-04-55-a186" src="https://github.com/user-attachments/assets/c072435c-c820-4d06-a3b4-b700bc323a23" />

[16.2.9.2.6. 현재 디렉토리 경로(Current Directory Path) - 그림 90.4.55.a186](./16-02-09-02-06-current_directory_path.md#90-04-55-a186)

<a id="90-04-55-a187"></a>

#### 그림 90.4.55.a187. `이미지 저장` 대화상자 → `새 폴더 만들기` (Windows) (우리말)
<img width="707" height="619" alt="90-04-55-a187" src="https://github.com/user-attachments/assets/909743be-f027-459e-ac1b-a6731bb6210b" />

[16.2.9.2.7. 새 폴더 만들기(Create Folder) - 그림 90.4.55.a187](./16-02-09-02-07-create_folder.md#90-04-55-a187)

<a id="90-04-55-a188"></a>

#### 그림 90.4.55.a188. `이미지 저장` 대화상자 → `파일 유형 선택` (Windows) (우리말)
<img width="707" height="619" alt="90-04-55-a188" src="https://github.com/user-attachments/assets/3d0ad8fa-6250-4672-bd9d-ad40a1f38bdf" />

[16.2.9.2.8. 파일 유형 선택(Select File Type) - 그림 90.4.55.a188](./16-02-09-02-08-select_file_type.md#90-04-55-a188)

***

## 다른 페이지로 가기

[➡️ 다음: 90.4.56. 레이어 합치기(Merge Layers)](./90-04-0056-merge_layers.md)

[⬅️ 이전: 90.4.54. 이미지를 붓 파이프로 내보내기(Export image as brush pipe)](./90-04-0054-export_image_as_brush_pipe.md)

[⬆️ 위: 90.4. 대화상자(Dialogs)](./90-04-0000-dialogs.md)

[⬆️ 위: 90. 이미지 맵](./90-00-image-map.md)

[🏠 홈](./00-home.md)
