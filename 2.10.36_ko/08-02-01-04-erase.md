...
`지우기(Erase)` 모드에서 위쪽 [레이어](./19-glossaryx-layer.md)에 [레이어 마스크](./19-glossaryx-layer_mask.md)를 쓰고 있다면, 레이어 마스크의 효과가 반대로 나타납니다. 아래 영상에서 레이어 마스크에 검은색으로 칠할 때, 결과 이미지가 투명에서 원래의 이미지로 돌아오는 것을 확인할 수 있습니다.
