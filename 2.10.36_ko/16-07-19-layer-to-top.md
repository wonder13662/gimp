...
## 1. 기본 동작
`레이어를 꼭대기로(Layer to top)` 명령은 [활성화 레이어](./19-glossaryx-active_layer.md)를 [레이어 스택](./19-glossaryx-layer_stack.md)의 가장 위로 올립니다.

활성화 레이어가 [레이어 스택](./19-glossaryx-layer_stack.md)의 맨 위거나, 유일한 [레이어](./19-glossaryx-layer.md)라면 `레이어를 꼭대기로(Layer to top)` 명령은 비활성화됩니다.
