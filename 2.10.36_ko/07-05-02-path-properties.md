# 7.5.2. 경로 속성
경로는 [레이어](./19-glossaryx-layer.md)나 [채널](./19-glossaryx-channel.md)처럼 이미지의 구성 요소입니다. 이미지가 GIMP(김프)의 고유한 파일 포맷인 [`XCF`](./19-glossaryx-xcf.md)으로 저장되면, 이미지가 가지고 있던 경로도 이미지와 함께 저장됩니다.
