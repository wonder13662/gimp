# 14.3.13.2. 동작 변경키(Key modifiers)
...
Ctrl(⌘) 키를 누른채로 이미지창에서 클릭해서 이미지의 활성화된 레이어로부터 복원할 수 있습니다. `도구 옵션` → `복원` → `정렬(Alignment)`의 값이 [없음(None)]() 또는 [정렬됨(Aligned)]()이면, 클릭한 지점이 복원의 원본 시작점이 됩니다.
