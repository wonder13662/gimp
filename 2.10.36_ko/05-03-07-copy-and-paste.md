...
[이미지 메뉴 바](./19-glossaryx-image_menu_bar.md)의 `파일` → `만들기` → `클립보드에서` 메뉴를 눌러 클립보드에 저장된 이미지로 새로운 이미지를 만듭니다.
...
[이미지 메뉴 바](./19-glossaryx-image_menu_bar.md)의 `편집` → `붙여넣기(고급)` → `새 이미지` 메뉴를 눌러 클립보드에 저장된 이미지로 새로운 이미지를 만듭니다.
