# 14.2.1.1. 키 변형자(Key modifiers)
선택 도구를 사용하는 도중에 `키 변형자(Key modifiers)`인 Ctrl(⌘), Alt(⌥), Shift(⇧) 등을 눌러 선택 도구의 동작을 변경할 수 있습니다.
...
#### 영상 14.2.1.1.a201. Alt(⌥) 키를 눌러 선택 옮기기
...
기존 선택 영역에서 새로 선택된 영역을 더하는 모드(Addition Mode)로 동작합니다.
...
사용자가 어떤 도구를 사용하고 있느냐에 따라 달라집니다. 예를 들어 [사각 선택 도구](./14-02-02-rectangle-selection.md)의 경우, 정사각형의 선택을 만들 수 있습니다.
...
#### [영상 90.2.7.a31.2. `기본 설정` → `이미지 창` → `스페이스 바` → `스페이스 바를 누르고 있는 동안` → `화면 이동` (Mac)](./90-02-07-00-image-windows.md#90-02-07-a31-02)
...
#### [영상 90.2.7.a31.3. `기본 설정` → `이미지 창` → `스페이스 바` → `스페이스 바를 누르고 있는 동안` → `이동 도구로 전환` (Mac)](./90-02-07-00-image-windows.md#90-02-07-a31-03)
