...
GIMP(김프)에서는 이미지들이 각각의 독립된 창으로 열립니다. 그리고 시스템 자원가 허용하는 한 많은 이미지를 동시에 열 수 있습니다. GIMP를 이용하여 이미지를 편집하려면, 우선 이 이미지 창을 이용해 그림을 불러와야 합니다. 이미지 창의 한쪽 구석에는 메뉴 모음(파일, 편집, 선택 등…)이 있으며, 이 메뉴 모음은 마우스 버튼 우클릭으로도 사용할 수 있습니다.
