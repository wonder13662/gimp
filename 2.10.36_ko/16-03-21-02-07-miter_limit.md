...
2개의 [경로의 선분](./19-glossaryx-segment.md)이 만날 때, [선결합 모양새(Join Style)](./16-03-21-02-06-join_style.md)가 연귀 맞춤(Miter)되는 정도를 `뾰족한 선결합 제한(Miter Limit)` 에서 설정합니다.

2개의 [경로의 선분](./19-glossaryx-segment.md)의 각도가 예리하지 않다면, 연귀 맞춤(Miter)은 생기지 않습니다.

`뾰족한 선결합 제한(Miter Limit)`은 2개의 [경로의 선분](./19-glossaryx-segment.md)을 따라 그려진 선의 외곽선이 연장되었을 때의 공간이 얼마만큼 채워지는지를 결정합니다.
...
아래 이미지에서는 외곽선 연장은 하얀색 선으로, 빈공간은 연두색으로 표시하고 있습니다.
