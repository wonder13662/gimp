...
붓 동적 요소는 여러 입력 동적 요소들에 다른 붓 매개변수를 지정해 줍니다.

붓 동적 요소는 대부분 펜 태블릿과 함께 사용합니다. 

하지만, 몇 가지의 붓 동적 요소는 마우스로도 쓸 수 있습니다.
...
`동적 붓으로 에뮬레이트(Emulate brush dynamics)` 옵션은 스트로크를 그릴 때, 붓의 압력(Pressure)과 속도(Velocity)를 스트로크의 길이에 따라 다르게 적용합니다. 

압력은 0에서 시작해서 최대 압력까지 올라간 뒤, 다시 내려오기 시작해서 0으로 돌아옵니다. 

속도는 0에서 시작해서 최대 속도까지 올라간 뒤, 다시 내려오기 시작해서 0으로 돌아옵니다.
...
<a comment="TODO 14.3.2. 동적 요소 관련 내용과 예시 추가할 것"></a>
