# 15.2.2.2.3.4. 채널 이름(Channel Name)

<a id="90-04-04-a11-04-04"></a>

#### [그림 90.4.4.a11.4.4. `채널 대화상자` → `채널 속성` → `채널 이름` (Windows) (우리말)](./90-04-0004-channels.md#90-04-04-a11-04-04)
<img width="262" height="290" alt="90-04-04-a11-04-04" src="https://github.com/wonder13662/gimp/assets/15767104/ec055898-7206-4bb2-bc0e-dc55967fc65b" />

채널의 이름을 나타냅니다.

선택 마스크 채널의 이름을 더블 클릭하면, 이름을 직접 수정할 수 있습니다.

채널의 이름은 이미지 안에서 고유해야 합니다. 같은 이름을 강제로 쓸 경우, 접미사로 번호가 붙습니다.

<a id="90-04-04-a11-04-04-01"></a>

#### [영상 90.4.4.a11.4.4.1. `채널 대화상자` → `채널 속성` → `채널 이름`: 중복된 채널 이름에는 맨 뒤에 번호가 붙습니다 (Windows) (우리말)](./90-04-0004-channels.md#90-04-04-a11-04-04-01)
<video controls="controls" width="640" height="360" src="https://github.com/wonder13662/gimp/assets/15767104/798680d8-48b6-423b-b168-05654e5483f1"></video>

원색 채널과 [알파 채널](./19-glossaryx-alpha_channel.md)의 이름은 수정할 수 없습니다.

미리보기를 더블 클릭해서 `채널 속성` 대화상자를 열어 채널의 속성인 이름을 수정할 수 있습니다.

아래 영상에서 `채널 미리보기`를 더블 클릭해서 `채널 속성` 대화상자를 열어 채널의 속성인 이름을 바꾸는 과정을 확인할 수 있습니다.

<a id="90-04-122-a101-01"></a>

#### [영상 90.4.122.a101.1. `채널 속성` 대화상자 → `채널 이름`: 이름 바꾸기 (Windows) (우리말)](./90-04-0122-channel_attributes.md#90-04-122-a101-01)
<video controls="controls" width="640" height="360" src="https://github.com/wonder13662/gimp/assets/15767104/dbddd7c0-4019-458a-820e-bd87c6f9b319"></video>

***

## 다른 페이지로 가기

[➡️ 다음: 15.2.2.2.4. 채널 관리하기(Managing Channels)](./15-02-02-02-04-00-managing_channels.md)

[⬅️ 이전: 15.2.2.2.3.3. 미리보기(Thumbnail)](./15-02-02-02-03-03-thumbnail.md)

[⬆️ 위: 15.2.2.2.3. 채널 속성(Channel Attributes)](./15-02-02-02-03-00-channel_attributes.md)

[⬆️ 위: 15.2.2.2. 채널 대화상자 사용하기(Using the Channel Dialog)](./15-02-02-02-00-using_the_channel_dialog.md)

[⬆️ 위: 15.2.2. 채널 대화상자(Channel dialog)](./15-02-02-00-channel_dialog.md)

[⬆️ 위: 15.2. 이미지 구조 관련 대화상자(Image structure related dialogs)](./15-02-00-image-structure-related-dialogs.md)

[⬆️ 위: 15. 대화상자(Dialogs)](./15-00-dialogs.md)

[🏠 홈](./00-home.md)

[원문](https://docs.gimp.org/2.10/ko/gimp-channel-dialog.html#gimp-channel-dialog-using-attributes)