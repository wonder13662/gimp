...
이미지의 [`EXIF`](./19-glossaryx-exif.md) 데이터를 확인하고 싶다면, [16.6.36. 메타데이터 뷰어](./16-06-36-metadata-viewer.md) 플러그인을 사용할 수 있습니다.

[`이미지 메뉴 바` → `이미지` → `메타데이터` → `메타데이터 조회`](./16-06-35-metadata.md) 메뉴를 선택하면 됩니다.
