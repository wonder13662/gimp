...
`닷지(Dodge)` 모드는 아래쪽 레이어의 픽셀의 [RGB](./19-glossaryx-color_mode_rgb.md) 값을 위쪽 레이어의 반전시킨 픽셀의 RGB값으로 나눈 것입니다. 
...
하지만 그레이스케일의 이미지에서 이러한 용도로 사용한다면, 레이어 모드보다 페인팅 도구 중 [닷지(Dodge)/번(Burn)](./14-03-17-00-dodge-burn.md) 도구를 사용하는 것이 좋습니다.
