...
## 1. 문제
사전에 [이미지](./19-glossaryx-image.md)의 일부분을 선택했을 수도 있지만, 지금은 작업하고 있는 부분이 선택 영역의 안쪽이 아닙니다.

[선택 영역](./19-glossaryx-selection.md)의 테두리를 찾아 작업하려던 곳이 맞는지 확인해야 합니다.
...
[`이미지 메뉴 바` → `선택` → `없음`](./16-04-03-00-none.md) 또는 [키보드 단축키 Ctrl(⌘) + Shift(⇧) + `A`](./16-04-03-01-activating_the_command.md)를 누릅니다.
...
선택영역이 보이더라도, 기대하는 것과는 정반대의 선택영역일 수도 있습니다. 

가장 쉽게 확인하는 방법은 [퀵 마스크](./07-03-00-the-quickmask.md) 버튼을 눌러보는 것입니다. 

선택 영역인 곳은 사라지고, 선택 영역이 아닌 곳은 가려집니다. 
...
확인된 선택영역이 올바르지 않다면, 퀵 마스크를 끈 뒤에 [`이미지 메뉴 바` → `선택` → `반전`](./16-04-04-00-invert.md) 또는 단축키 Ctrl(⌘) + `I`를 눌러 선택영역을 반대로 바꿀 수 있습니다.
