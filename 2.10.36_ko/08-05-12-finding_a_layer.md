...
많은 수의 레이어를 작업할 때, 목록에서 특정 레이어를 찾는 것은 쉽지 않습니다. 

GIMP(김프) 2.10.10 이후부터 [캔버스](./19-glossaryx-canvas.md)에 직접 레이어를 선택하는 기능이 추가되었습니다. 

Alt(⌥)를 누른 채로 마우스 휠(Mouse Middle Button)로 클릭하면 엘리먼트의 레이어를 선택(활성화)할 수 있습니다.
...
만일 같은 위치에 여러 레이어가 있다면 가장 위의 레이어부터 차례대로 활성화됩니다. 

또한 활성화된 레이어의 이름이 [상태 표시 영역](./03-02-04-09-status-area.md)에 나타납니다.
