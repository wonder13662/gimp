...
[Scheme](https://en.wikipedia.org/wiki/Scheme_(programming_language))은 Lisp 계열의 프로그래밍 언어의 한 종류입니다. 

GIMP(김프)는 가벼운 인터프리터 언어인 `TinyScheme`을 사용합니다. 

`TinyScheme`은 `R5RS` 표준을 따릅니다.

아래 3가지 기본 규칙을 확인해주세요.

<a id="13-03-01-01-s1"></a>
...
<a id="13-03-01-01-s2"></a>

## 2. 괄호 안의 아이템의 순서
괄호 안의 첫번째 아이템은 함수(function)입니다. 

그리고 나머지 아이템은 함수의 인자(parameter)들입니다.

하지만, 괄호안에 포함된 것이 함수가 아닐 수도 있습니다. 
...
왜냐하면, 함수가 앞에 놓이기 때문입니다.

아래 예시에서 괄호로 감싼 아이템들 중, 첫번째 아이템인 `+` 연산자는 함수입니다. 

두번째 아이템 `3`, 세번째 아이템 `5`은 인자입니다.

```
(+ 3 5)
```

<a id="13-03-01-01-s3"></a>
